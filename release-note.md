...
*   We added an 'Explain Code' button that provides clear explanations for generated R scripts. Users can now easily understand the underlying R code generated by their operations, helping them learn R, verify data transformations, and share analytical logic with confidence.

### Data Source

*   We enhanced OAuth connection handling to automatically detect and correct invalid connection references when publishing dashboards.

### Common

*   We added the ability to resize the column selection pane in the Column Selection Dialog via drag-and-drop. Users can now expand the pane to view long column names clearly, enhancing usability and navigation in complex datasets with many columns.

### Data Wrangling

*   We added visual distinction, such as a distinct color, between parent data frames and branch data frames in the Step Diagram.

### Chart

*   We added a Row Count Settings dialog for Pivot Tables using multiple-answer columns to support displaying the original row count for the CNT measure. Users can now choose to show the original pre-split row count instead of the expanded count, making it much easier to report accurate respondent counts in survey analyses.
*   We enabled column-width resizing via dragging within the expanded views for Pivot Tables, Summary Tables, and Tables.

### Analytics

*   Correlation We improved the variables grouping logic in the Heatmap chart. Not the the variables are grouped and ordered based on the close proximity of the correlation coefficients.&#x20;
*   LCA: We added support for selecting numeric target columns in Latent Class Analysis (LCA). It will automatically convert to factor data type (Ordinal Categorical data).&#x20;
*   Decision Tree: We added the ability to copy the full condition text directly from decision tree tooltips.&#x20;
*   K-Medoids: We updated the default setting of "Show All Variables" to TRUE for the K-Medoids Cluster Profile radar chart so all clustering variables are displayed by default.
*   We standardized the P-value and percentage formatting within the Analytics Report body to show values smaller than 0.001 as "< 0.001" and "< 0.1%".&#x20;



### Note

*   We added an option to disable the automatic page break before H1 headings when exporting notes to PDF. Users can now prevent unwanted blank spaces or pages containing only titles.

## Issue fix

### Summary View

*   In Summary View, the Show Details / Hide menu failed to work and threw an assertion error for numeric columns containing only 0 and 1.

### Data Wrangling

*   In the Row Filter dialog, the duplicate icon was placed in an incorrect position.
*   When adding a new step via the Set Value Order dialog, the newly created column was incorrectly placed at the far left instead of immediately to the right of the selected column.
*   In the Step Diagram, newly created branches failed to appear immediately, the controller overlapped the dialog footer, and connecting node lines were too short to display arrows properly.
*   In the Step Diagram, toggling the visibility of disabled steps caused step nodes to misalign and appear far away from their intended positions.

### Chart

*   In Pivot Tables, clicking Show Detail on a row header cell with an NA value showed zero rows because the generated filter condition compared the column against a literal string rather than using an NA-aware operator.
*   Upgrading Exploratory caused existing pivot table color palettes to unintentionally change from blue to green because the default palette was updated. We've updated it so that the pivot tables that were created in the previous version (v15.x or before) will not uptake the new default color palette.&#x20;

### Analytics

*   In Ordinal Logistic Regression, the chart color changed unexpectedly when switching chart orientation, and the chart was not vertical by default.
*   The z-value statistic label was not properly translated in the UI.

### Parameter

*   When using an equal\_or\_all session or dashboard parameter filter, selecting an empty or NULL value caused a hard crash with a size-0 vector error instead of treating it as all values or handling it gracefully.
...
*   Common R functions like `read_csv()` were not being correctly syntax-highlighted in the R Script editor within the AI Data dialog.
*   The Snowflake ODBC driver auto-setup failed because the vendor (Snowflake) changed the package naming convention and driver internals.
