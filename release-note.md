...
* Data Source: CSV: When encoding detection fails, it should fallback to CP932 or Shift JIS for Japanese CSV files.
* Data Source: Excel: When importing and merging multiple Excel Sheets by unchecking some of columns, the "id" column disappeared.
* Data Source: Google Sheets: Importing data failed with error in curl::curl_fetch_memory(): ! Error in the HTTP2 framing layer for some occasion.
* Data Source: JSON: It should remember the last selected node in the schema tree.
* Data Source: Salesforce: Comma separated values in a 'longtext' field was treated as numeric where it should be character.
* Data Source: Updating SQL Data Source failed with "Cannot read properties of undefined (reading 'get')" error for some occasion.
* Data Source: After deleting a data frame created with "Save As New Data Frame" operation, you could not restore it from Change History.
...
* Data Wrangling: Filter token does not show correct label for numeric is in operator
* Data Wrangling: On Windows  a custom R Command failed with error code 127 for a long script.

* Table View: Metrics popup failed to show the content if you opened the metrics popup by clicking the "i" icon in the table header right after the project open.
...
* Analytics: Confidence interval value was different between Statistics tab and Mean tab in the Wilcoxon Test. 
* Analytics: Hovering the table header of the Summary tab in the Chi-Square A/B test didn't show the "i" icon for the help.
...
* Analytics: When you updated a step in analytics view and run analytics, the analytics did not reflect the updated data.

* Note: Word export is disabled.

* Parameter: When creating a new parameter from the Filter Dialog, now it uses the Filter operator, column information, etc to set default parameter settings.
...
* Publish: When publishing a chart, Notify Team Member parameter is not passed to server
...
* History: It did not show the saved data frame name for "Save As Data Frame" case. 
