...
* データソース: データコネクションのダイアログで、「標準SQLを利用する」オプションを明示的にチェックをせずにデータソースを作成した場合、サーバー上では標準SQLが利用されませんでしたが、デフォルトで標準SQLを利用するようになりました。
...
* インサイト: インサイトの検索ビューの表記が日本語になりました。
* インサイト: 「実行環境の設定」メニューの内容を「アカウントの設定」ページに統合しました。
* インサイト: コラボレーション・サーバーにログインしていないときに、インサイトやプロジェクトのページに、「ログイン」ボタンを表示しました。
* インサイト: ノートのスケジュールを実行すると、特定の条件下で、 "Cannot read property 'forEach' of null. "というエラーが表示される。


* 一般: データフレーム名にマルチバイト文字（日本語）が使用できるようになりました。
...
* パラメーター: パラメータの「値のリスト」の生成に必要な列がない場合のエラーメッセージを改善しました。
* パラメーター: インサイトを作成した人以外がインサイトを更新すると、パラメーターを実行するときに、データベースの接続に関するエラーが表示される。


* スケジューラー: チームとして共有したダッシュボードのスケジュールを止めても、サーバーにスケジュールの設定が残り続ける。
