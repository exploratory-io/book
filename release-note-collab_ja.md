...
* [新規のインストールの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-16.0.62.tar.gz)  ([インストールガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-TSP7qbs5td))
* [アップグレードの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-images-16.0.62.tar.gz) ([アップグレードガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-SEH3IFi8cG))

## 機能強化

### チャート

*   チャートの画像エクスポートを無効にするオプションを追加しました。作成者は、データのダウンロードオプションと併せて、チャートを画像としてエクスポートできるかどうかを選択できるようになりました。
*   チャートエディタの起動パフォーマンスを改善し、従来よりも約7〜8倍高速化されました。

### ダッシュボード

*   Parquetファイルの読み込みを最適化することで、インタラクティブモードの起動を改善しました。これにより、大規模なデータセットを含む複雑なダッシュボードを開く際の待ち時間が短縮されます。開始時間が85%ほど高速化されるのを確認しています。
*   LOV（値のリスト）のキャッシングやRの評価のバッチ処理など、インタラクティブモードのパラメータに関する多段階のパフォーマンス改善を実施しました。これにより、特に選択肢のリストが膨大な場合に、パラメータの更新待ち時間が大幅に短縮されます。
*   LOVの更新方法を最適化することで、連動するパラメータ（カスケードパラメータ）のパフォーマンスを改善しました。LOV専用のデータフレームを別途作成することなく、大規模なデータセットでも高速なパラメータ更新が可能になりました。
*   カスケード・パラメータ（従属関係にあるパラメータ）を使用している場合、パラメータ値の更新速度が20〜30倍向上します。

### スケジュール

*   スケジュールのオプションに、営業時間内のみ実行するなど、特定の時間を指定できる機能を追加しました。

## 不具合の修正

### アナリティクス

*   サーバー上でアナリティクスの相関テーブルを画像としてエクスポートすると、実際の画像データの代わりに0バイトの空ファイルが生成される問題を修正しました。

### チャート

*   埋め込みコンテンツの設定で全画面表示や画像エクスポートの機能をオフにしても、それらのボタンが表示されたままになる問題を修正しました。

### ダッシュボード

*   サーバー上で公開されたダッシュボードにアクセスした際、読み込みが開始される前に空白のページが表示され、大幅な遅延が発生する問題を修正しました。
*   チャートをクリックしてインタラクティブモードを開始しようとすると、「Rプロセスが実行されていません」というエラーで失敗する問題を修正しました（パラメータペインからの開始は正常に動作していました）。
*   サーバー上の特定のダッシュボードでチャートの値をクリックすると「エラーコード 127」が発生する問題を修正しました。
*   ダッシュボードのエラーダイアログで「OK」ボタンをクリックした際、即座にフィードバックが返されず、ページがリロードされるまでに遅延が生じる問題を修正しました。
*   スケジューラ側でサムネイルのキャプチャが正しく動作していなかったため、スケジュール通知メール内のスクリーンショットがサーバー上の実際のダッシュボードと異なる数値を示していた問題を修正しました。

### パラメータ

*   セッションフィルタが適用されたパラメータで「値をリセット」ボタンをクリックすると、現在のユーザーに閲覧権限のないデフォルト値が表示されてしまう問題を修正しました。
*   パラメータペインの「値をリセット」リンクをクリックした際、依存関係にある連動パラメータの自動更新がトリガーされない問題を修正しました。
*   ホストサーバー上のインタラクティブモードにおいて、ダッシュボードを再度開いた後にパラメータが正しく反映されず、一部のチャートでデフォルト値が表示されたりデータが表示されなかったりする問題を修正しました。
*   単一選択パラメータにおいて、不要な場面で「(選択なし)」オプションが表示されたり、「(選択なし)」で実行するとリストの最初の値が自動的に選択されたりする問題を修正しました。
*   デフォルトが「選択なし」に設定された単一選択パラメータをサーバー上でリフレッシュすると、利用可能な最初の値が自動的に選択されてしまう問題を修正しました。

### 管理画面

*   サーバー上でチームにメンバーを追加した際、実際にはページのリロード後にメンバーが追加されているにもかかわらず、「内部サーバーエラー」メッセージが表示される問題を修正しました。
...
* [新規のインストールの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-15.0.5.tar.gz)  ([インストールガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-TSP7qbs5td))
* [アップグレードの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-images-15.0.5.tar.gz) ([アップグレードガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-SEH3IFi8cG))


### 機能強化

* コンテンツ: Rカスタムチャートを含む、パブリッシュされたチャート、ノート、ダッシュボードを表示できるようになりました。
* スケジュール: AI関数のステップを含むコンテンツをスケジュール実行できるようになりました。
* スケジュール: Rカスタムチャートを含むコンテンツをスケジュール実行できるようになりました。

### 修正された問題

* コンテンツ: パブリッシュ済みノートの設定ダイアログからサムネイル画像を更新しようとすると、「パラメーターの設定のアップデートに失敗しました」というエラーで更新できない問題を修正しました。
* コンテンツ: SankeyチャートやCircular Chord DiagramなどのRコードでd3.jsを使って描画したチャートが、パブリッシュされたノートおよびダッシュボード上で正しく表示されない問題を修正しました。
* コンテンツ: アカウントを別のユーザーに引き継いだ後、別のユーザーがプロジェクトを開いていることを示す緑色のインジケーターランプが消えずに点灯し続ける問題を修正しました。
* コンテンツ: パブリックプランのユーザーにパブリッシュ済みダッシュボードを共有し、「共有設定の変更可能」の権限を付与しても、対象のビューアーに共有ボタンが表示されない問題を修正しました。
* コンテンツ: 共有設定にメールアドレスをカンマ区切りで貼り付けた際、すでにアクセス実績のあるユーザーが登録対象から除外されてしまう問題を修正しました。
* コンテンツ: データカタログのパブリッシュ時に、ステップのフラグがクリアされないために不要なparquetファイルの再生成が発生し、大きなデータでタイムアウトエラーが発生する問題を修正しました。
* コンテンツ: パブリッシュ済みのノートおよびダッシュボード上のインタラクティブチャートからエクスポートしたCSVファイルにUTF-8 BOMを付与し、Excelで開いた際に文字化けしないようにしました。
* コンテンツ: インサイト検索ページのコンテンツタイプフィルターから「すべて」のオプションを削除し、デフォルトを「ノート」に変更しました。
* データソース: BigQueryのサービスアカウントによるデータインポートにおいて、クエリーが失敗した際にエラーを出さずにnullを返してしまい、ユーザーが失敗に気づけない問題を修正しました。
* 管理: ホステッドサーバーのユーザー管理画面のユーザー一覧テーブルで横スクロールバーが常に表示されるようになり、「ユーザー削除」などの操作が見つけやすくなりました。
...
* [新規のインストールの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-14.2.1.tar.gz)  ([インストールガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-TSP7qbs5td))
* [アップグレードの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-images-14.2.1.tar.gz) ([アップグレードガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-SEH3IFi8cG))


### 機能強化

* コンテンツ: ダッシュボードおよびノートを「PDFにエクスポート」メニューからPDFとしてエクスポートできるようになりました。
* コンテンツ: 設定ダイアログから、コンテンツのCSVファイルのダウンロードを制限できるようになりました。
* コンテンツ: 設定ダイアログから、コンテンツのEDFファイルのダウンロードを制限できるようになりました。
* コンテンツ: ノートおよびダッシュボード内のマップチャートの画像エクスポートに対応しました。

### 修正された問題

* AI: クォータ情報の取得のみが行われた場合にAIクォータのリセット日が更新されず、「利用量が月間プランの上限を超えています」というエラーが誤って表示される問題を修正しました。
* インタラクティブ: インタラクティブモードのオン・オフを素早く切り替えると、サーバー側でコンフリクトエラーが発生する問題を修正しました。
* パラメーター: パラメーターを変更した後にチャートデータをエクスポートすると、変更が反映されずに初期値のデータがエクスポートされる問題を修正しました。
* パラメーター: v14.1へのアップグレード後、デフォルト値が正しくセットされない、子パラメーターの値リストが動的に更新されないなど、パラメーターの動作に問題がある問題を修正しました。
* パラメーター: セッションパラメーターに依存するパラメーターを使用した場合、サーバーでのダッシュボードの実行結果がExploratoryデスクトップと異なる問題を修正しました。
* パラメーター: 日付パラメーターがユーザーが選択した値ではなく、デフォルト値に強制的に戻される問題を修正しました。
...
* コンテンツ: 設定ダイアログから、ノートおよびダッシュボードにカスタムのサムネイルをアップロードできるようになりました。
* コンテンツ: インタラクティブモードで、設定メニューからテキストパネルの編集アイコンを有効または無効にできるようになりました。



* インタラクティブ: ログインしているユーザーのセッションID（メールアドレス、名、姓、またはチーム）でデータをフィルタリングするために、ユーザーセッションコンテキストをパラメーターとして使用できるようになりました。
* インタラクティブ: 冗長な呼び出しを回避することで、インタラクティブセッションのパフォーマンスを改善しました。


### 修正された問題

* 一般: プロフィール画像を変更できない問題を修正しました。
* コンテンツ: ビューアーがダッシュボードのテキストエリアを編集できないように修正しました。
* コンテンツ: OAuth トークンが期限切れの場合、Googleスプレッドシートにエクスポートするスケジュールが有効になっているコンテンツを開くと、OAuth ダイアログが自動的に表示される問題を修正しました。
* パラメーター: インタラクティブモードを開始した直後、パラメーターを変更せずに実行した場合、クライアントのパラメーター選択がサーバーの状態と異なる場合にチャートが更新されない問題を修正しました。
* スケジュール: Rコードを含むダッシュボードをスケジュールした際に、"SyntaxError: Unexpected token '??='" エラーでスケジュールが失敗する問題を修正しました。
...
* [新規のインストールの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-14.0.4.tar.gz)  ([インストールガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-TSP7qbs5td))
* [アップグレードの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-images-14.0.4.tar.gz) ([アップグレードガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-SEH3IFi8cG))


### 機能強化

* AI: ホステッドサーバー上で「AI 関数」「AI エディター」機能を利用できるようになりました。  

### 修正された問題


* コンテンツ: ログアウト状態で複数タブを開いていると、ログイン時に最後に開いたタブが常に表示されていたという問題を修正しました。
* コンテンツ: コンテンツタイプの検索条件のドロップダウンに「AI 関数」と「AI エディター」が無いという問題を修正しました。
* コンテンツ: 列数の多いデータを公開するとサマリ・タブが空白になっていたという問題を修正しました。
* コンテンツ: ダッシュボード内で、 繰り返しの機能を使用したチャート上部の「Number of Columns」ラベルが現在の UI言語設定に従っていなかったという問題を修正しました。


* インタラクティブ: 以前にチャートからピンされていたステップがダッシュボード内ある場合に「Failed to load R data」エラーでインタラクティブ・セッションの初期化に失敗していたという問題を修正しました。
* インタラクティブ: コンテンツがチーム向けにパブリッシュされていて、データキャッシュが有効な場合に「Failed to load R data」エラーでインタラクティブ・セッションの初期化に失敗していたという問題を修正しました。


* パラメーター: カスケードパラメーターが動作していなかったという問題を修正しました。
* パラメーター: ダッシュボード内で同じパラメーター設定でも、複数のワードクラウドが同じ結果にならなかったという問題を修正しました。
* パラメーター: ダッシュボード内でチャート数が多い場合に、一部のチャートが更新されていなかったという問題を修正しました。
* パラメーター: アップデートしたパラメーターがSQLクエリに関連していないにもかかわらず、SQLクエリが実行されていたという問題を修正しました。
* パラメーター: 数値型のレンジスライダーのパラメーターの開始値が特定条件下で NaN になり操作できなくなっていたという問題を修正しました。

* 管理: 一度設定したチーム説明を削除できなかったという問題を修正しました。
* 管理: アクセスログのページの、日付の範囲フィルターの結果が1日ずれていたという問題を修正しました。
...
* コンテンツ: 「URLでの共有」が設定されているコンテンツにも'noindex' タグを追加しました。このアップデートにより、もしGoogleなどの検索エンジンにアクセス可能なパブリックのページに該当のコンテンツのURLが記載されていたとしても、検索の結果には出てこないようになります。
...
* [新規のインストールの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-13.2.1.tar.gz)  ([インストールガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-TSP7qbs5td))
* [アップグレードの場合](https://exploratory-download.s3-us-west-2.amazonaws.com/collab-server/exploratory-server-images-13.2.1.tar.gz) ([アップグレードガイド](https://exploratory.io/note/exploratory/Exploratory-Collaboration-Server-SEH3IFi8cG))
    * Exploratory Serverのバージョン13以前からアップグレードする場合は、**「11. Exploratory サーバー をv13にアップグレードする場合 (MongoDBのバージョンのアップグレード」** のステップを忘れずに実施してください。
