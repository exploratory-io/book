...
いったんデータをインポートすると、サマリー画面で、データの概要を見ることができます。矢印で指した場所では、カラム数と行数を確認することができます。今回のデータでは、25カラムと445,827行と表示されています。また、他のカラムも簡単に見ることもできます。例えば、CARRIERカラムでは頻繁に出てくる航空会社を見ることができます。
...
分析したいカラムだけを選んで見るためにはたくさんの方法があります。では、いくつか試してみましょう。
...
あなたは、既存のselectコマンドを更新したり、新しいselectステップを加えることができます。下記のように、既存のselectのステップを更新してみましょう。次は、ORIGINで始まるカラムを取り除いてみます。select()コマンドの中にstarts_with()関数を使うと、starts_with()関数の引数で指定した文字から始まるカラムだけを選ぶことができます。select()関数の中に、レコメンドされたリストから','（カンマ）や'-'を入力し始めることができます。その次に、リストからstarts_with()関数を選んでください。
...
Exploratoryでは、あなたは、簡単に新しいカラムを作って、計算をしたり文法を適用させることができます。
...
```
Runボタンを押して、画面の最下部のほうにスクロールするとweekdayという新しいカラムができていますね。

![](images/flight-weekday.png)

データが、1~7という数字として表示されているのがわかりますね。これは、```wday()```関数が曜日を意味する1~7の数値を与えているからです。1は月曜日を表しています。このままではわかりにくいですよね。下記のように```label```という引数を加えることによって、数値の代わりに、月曜日のような曜日の名前を取得することでより直感的に理解することができます。
...
それぞれのCARRIERごとにいくつの数が記録されているのかを計算するために```summarize()```コマンドの中に```n()```関数を使うとそれぞれのCARRIERごとにいくつの数が記録されているのかを計算することができます。
...
下記のように=を入力するとすぐに、データを集計するための関数のリストがレコメンドされます。
...
それぞれの航空会社がいくつの州に向かって飛んでいるのか気になりませんか？　下記のように```summarize```コマンドの中に```n_distinct()```関数を使うことでフライトの到着地を意味するDEST_STATE_ABRカラムの値を計算することができます。
...
新しく作られたcountカラムを使って、合計に対する比率を知りたくないですか？　```sum()```関数を使うと、合計を計算することができます。まずは、新しいカラムを作ることができる```mutate()```コマンドを使いましょう。```summarize()```は行をまとめて新しいカラムを作るときに使うのに対して、```mutate```は、この行のまま新しいカラムを作りたいときに計算をするときに使うことに注意してください。計算をしたいときは基本的にこの２つしか使いません。
...
新しいカラムの名前としてratioを入力し、次に=を入力してください。
...
チャート画面を見ると何が起きているかをビジュアライズ化することができます。それぞれの航空会社を表すバーの中に、それぞれのアメリカの州に対するフライトの数に基づいた比率を見ることができます。

![](images/flight-ratio-chart.png)

### Window Function - Rank

では、最後のステップです。これは、かなりクールでたくさんの可能性を秘めています。Window関数と呼ぶ人もいます。
