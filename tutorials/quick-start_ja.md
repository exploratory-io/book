...
プロジェクト内で、左側のペインの 「データフレーム」テキストの隣にある 「+」（プラス）ボタンをクリックし、ドロップダウンリストから 「ファイルデータ」を選択します。
...
この演習ではパラメータには何も入力せずに、そのまま「保存」ボタンをクリックしてください。名前もデフォルトのまま、作成ボタンを押します。
...
チャートは先ほどまでの文字のデータ型の時とは異なり、データが2016-08-01から2016-08-31まで時系列にどのように分布しているかを表すヒストグラムが表示されています。
...
これはExploratoryがバックエンドで実際に実行しているR言語のコマンドそのものです。データの整備・加工を行うたびに、ここにそのプロセスが「ステップ」として追加されていくので、これまでの処理手順を容易に確認することができます。
...
プロパティメニューから背景スタイルを変更することができます。

![](images/quick-start-108_1-ja.png)

たとえば、「ダーク(暗め)」タイプを選択すると、黒色のテーマを取得できます。
...
列の値は、曜日の自然順序を尊重して、「Sun」、「Mon」、「Tue」などのようにソートされています。(お使いのマシンの設定によっては日、月、火のように日本語で表示されます。) これは、以前に見たように 「day_of_week」列はfactorデータ型で、ソート順情報が埋め込まれているため、ピボットテーブルなどのグラフはデータのソート時にソート順情報を尊重するためです。
...
生の数字の代わりに、 「合計値の割合」として値を表示することができます。 「値」の横にある「メニュー」アイコンをクリックし、「表計算」を選択します。

![](images/quick-start-29-ja.png)

デフォルト設定では、 「合計値の割合」がピボットテーブルのすべての値のパーセンテージとして計算されます。しかし、各州または各曜日の割合を見たいかもしれません。たとえば、「方向」の設定を「行(左から右)」に変更すると、どの曜日に他の曜日よりも多くの便があるか見やすくなります。
...
ここで、Y軸に DEP_DELAY列を割り当てて、飛行出発遅延時間の傾向を確認します。デフォルトのデータ集計関数は 「合計値」に設定されていますが、これを 「平均値」のように変更することができます。
...
ここで、右側の「上位N / 下位N」ステップの直前の「結合(join)」ステップをクリックします。

![](images/quick-start-49-ja.png)

グラフにはこの「結合(join)」ステップの結果ではなく、「上位N」の結果が表示されていることに注意してください。

「+ (プラス)」ボタンをクリックして「グループ化」を選択します。
...
メインデータフレーム 「airline_delay-2016-08」の下に作成された「correlation」という新しいデータフレームが追加されました。
...
「予測変数」をクリックし、そして列選択のダイアログで「全て」をチェックします。 右の方にスクロールし、'CANCELLATION_CODE'のチェックを外します。これはこのCANCELLATION_CODE列の値の多くが欠損値だからです。'CANCELLATION_CODE'のチェックを外すと、自動的に「全て」のチェックは外れます。
...
## シンプルなマークダウンエディタでノートを作る

データ分析からインサイトを発見したら、ノートを作成して、パブリッシュして他のメンバーと共有します。この章では、どのようにExploratoryでノートを作成、パブリッシュできるかを見てみます。

ドキュメントの隣の'プラス'ボタンを押します。

![](images/quick-start-80-ja.png)

ポップアップダイアログで名前を入力し、作成ボタンをクリックします。

マークダウンエディタが起動されるので、以下のようにマークダウンのシンタクスで書いていくことができます。

![](images/quick-start-markdown_1-ja.png)

マークダウンからのシンタクスの詳細については、[Exploratoryのマークダウンからヘルプ](https://docs.exploratory.io/markdown/markdown-note.html)を参照してください。

マークダウンからのシンタクスに詳しくなくても、ツールバーのアイコンを使ってテキストをフォーマットしたり、イメージを入力したりできます。

![](images/quick-start-markdown_4-ja.png)


### フォーマットされた結果をプレビューする

マークダウンのフォーマット(ヘッダーテキスト、イメージ、箇条書き、等々)の結果を、実行ボタンをクリックするか、ツールバーのサイド・バイ・サイドのアイコンをクリックすることで、プレビューできます。

![](images/quick-start-markdown_5-ja.png)

### Vizやアナリティクスのビューで作成したチャートを挿入

このフライトデータを使って前の章で作成した、チャートを、ツールバーのチャートアイコンをクリックすることで、ノートに挿入することが出来ます。

![](images/quick-start-markdown_10-ja.png)

チャートを選択するダイアログが開きます。

![](images/quick-start-markdown_6-ja.png)

ダイアログでチャートを選択して、OKボタンをクリックするすると、チャートを埋め込むコードが挿入されます。

![](images/quick-start-markdown_7-ja.png)

チャートを埋め込んだノートをプレビューするには、実行ボタンをクリックするか、ツールバーにあるサイド・バイ・サイドのアイコンをクリックします。

![](images/quick-start-markdown_8-ja.png)

実行すると、以下のように結果を確認できます。

![](images/quick-start-markdown_9-ja.png)


### Rコードを挿入する

R言語でのプログラミングに馴染みのある方は、[ggplot2](http://ggplot2.tidyverse.org)のようなパッケージを使って、Rスクリプトを書くことで、データを可視化したりできます。

![](images/quick-start-markdown_11-ja.png)
...
ggplot(airline_delay_2016_08, aes(FL_NUM, color = CARRIER)) +
  geom_density()
```
````

実行ボタンをクリックするか、ツールバーのサイド・バイ・サイドのアイコンをクリックすることで、Rコードを実行して、結果を生成することができます。

![](images/quick-start-markdown_12-ja.png)

最後に、画面の上の方にある、再インポートボタンをクリックすると、このノートが参照する、全てのデータフレームの元のデータソースからデータを再インポートし、チャートをリフレッシュできます。このボタンを使うことによって、いちいち各データフレームに戻って、データを一つずつロードするといった面倒なことをしなくてもよくなります。

![](images/quick-start-markdown_13-ja.png)

マークダウンエディタではこの他にもいろいろな事ができます。詳細は[Exploratoryのマークダウンのヘルプページ](https://docs.exploratory.io/markdown/markdown-note.html)を参照してください。

----

おめでとうございます! これでチュートリアルは完了です。 これでExploratoryを使用して、データサイエンスの最新の技術進歩を最大限に活用し、データからより深い洞察を素早く見つけ出す方法を身につけました！

何か質問がありましたらお気軽に [Team Exploratory](mailto:support@exploratory.io)までご連絡ください。
