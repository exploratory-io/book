...
以下のプロットは、選択された変数の値（カテゴリ）の関係を可視化するために2次元空間上にそれぞれの値を配置したものです。近くに位置する値は関連性が強いことを示します。

{{category_plot}}

このプロットの見方は以下の通りです。

* 同じ方向に位置するカテゴリ同士は正の関連があります
* 反対方向に位置するカテゴリ同士は負の関連があります
* 原点（0,0）から離れているカテゴリほど特徴があります
* 原点（0,0）付近に位置するカテゴリは、特に際立った特徴がありません
...
以下の表は、次元ごとの各変数の重み（その次元が変数に与える影響の強さ）を表しています。重みは0-100の間をとり、各次元のカテゴリを全て足すと100になります。重みの値が大きいほど、そのカテゴリの値はその次元において重要な役割を果たしていることを意味します。
