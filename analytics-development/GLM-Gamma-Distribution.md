...
目的変数である<%= target %>と選択された予測変数との関係を調べるために一般化線形モデル（ガンマ分布）を使った予測モデルが作られました。
...
_現在の有意水準（P値）は<%= 100 * baseline_p %>% (<%= baseline_p %>)に設定されていますが、これはアナ
リティクスの「設定」より変更可能です。_

//ここから確認する。対数リンク関数を使っているため、解釈が異なる。例: 年齢の係数が0.016238であることは、「他の条件が同じ場合、年齢が1歳上がるごとに支払額が約1.6%増加する」ことを意味します。
