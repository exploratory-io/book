目的変数である<%= target %>と選択された予測変数との関係を調べるために一般化線形モデル（GLM）のガンマ分布を使った予測モデルが作られました。
...
## モデルの指標の詳細
* 疑似R2乗
  * 疑似R2乗はガンマ分布GLMにおける目的変数の値のばらつきのうち、このモデルに使われている予測変数によって説明される割合の近似値を示します。
...
ガンマ分布GLMでは、係数は対数リンク関数を通して解釈されます。それぞれの予測変数の係数はそのデータ型によって以下のように解釈できます。

<% variables.forEach(variable => { %>
<% if (variable.type == 'numeric') { %>
**数値型の場合：**

他の変数の値が一定だとしても、<%= variable.variable %>が1単位上がると、<%= target %>は約<%= Math.exp(variable.se) %>倍になります。
<% } else if (variable.type == 'logical') { %>
**ロジカル型の場合：**

他の変数の値が一定だとしても、<%= variable.variable %>がTRUEの場合、FALSEに比べ<%= target %>は約<%= Math.exp(variable.se) %>倍になります。  
<% } else { %>
**カテゴリー（文字列）型の場合：**

他の変数の値が一定だとしても、<%= variable.variable %>が"<%= variable.variable %>"の場合、ベースレベルである"<%= variable.base_level %>"と比べて<%= target %>は約<%= Math.exp(variable.se) %>倍になります。
<% }}); %>

### 注意点

これらの係数の値は、ガンマ分布GLMでは対数リンク関数を通して解釈されます。予測変数同士で単位が異なる場合には、それらの係数の値を使って<%= target %>との関係の強さを直接比べることはできません。予測変数間の関係の強さを比べたい場合は、上記の「選択された予測変数と<%= target %>の関係の強さ」を参照ください。
...
* 分布の適合性の確認：ガンマ分布は正の連続値に適したモデルですが、データの特性によっては他の分布（対数正規分布など）も検討する価値があるかもしれません。
