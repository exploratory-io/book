...
この検定では効果量の1つであるEta（イータ）2乗を使用します。これは、<%= explanatory1 %>や<%= explanatory2 %>、およびそれらの交互作用での<%= target %>のばらつきが、全体のばらつきに占める比率を表します。つまり、<%= target %>のばらつきの全体のうち、どの程度の割合を各要因で説明できているかを示す指標です。
