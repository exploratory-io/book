...
この検定では効果量の1つであるEta（イータ）2乗を使用します。これは、<%= explanatory1 %>や<%= explanatory2 %>、およびそれらの交互作用によって<%= target %>の平均値のばらつきの大きさの全体のばらつきに対する比率を表します。つまり、<%= target %>のばらつきの大きさが<%= explanatory %>によって何%説明できているのかを解釈できます。
