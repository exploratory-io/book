...
- **RMSE（平方平均二乗誤差）**: <%= rmse %>
  これは予測値と実際値の誤差の大きさを示す指標です。値が小さいほど予測精度が高いことを示します。

- **MAE（平均絶対誤差）**: <%= mae %>
  予測値と実際値の絶対的な差の平均です。こちらも値が小さいほど予測精度が高いことを示します。

- **MAPE（平均絶対誤差率）**: <%= mape_pct %>%
  この値は、予測値が実際の値から平均して約<%= mape_pct %>%しか乖離していないことを示しています。
  <% if (mape < 0.1) { %>
  これは非常に高い予測精度です。
  <% } else if (mape < 0.2) { %>
  これは良好な予測精度です。
  <% } else { %>
  この値は改善の余地があることを示しています。
  <% } %>

- **R²値**: <%= r_squared %>
  <% if (r_squared > 0.8) { %>
  この値は極めて高く、モデルが<%= target %>のばらつきの<%= r_squared_pct %>%を説明できていることを示しています。
  <% } else if (r_squared > 0.5) { %>
  この値は良好で、モデルが<%= target %>のばらつきの<%= r_squared_pct %>%を説明できていることを示しています。
  <% } else { %>
  この値はモデルの精度向上の余地があることを示しています。
  <% } %>
...
このチャートは、実測値（青線）と実測値をもとにして作られた将来の予測値（オレンジ線）をラインチャートを使って可視化しています。予測期間は<%= period %> <%= unit %> 後までを予測しており、予測値のラインの薄い幅は、不確実性区間を表しています。
