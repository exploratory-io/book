...
- **MAPE（平均絶対誤差率）**: <%= mape * 100 %>%
  この値は、予測値が実際の値から平均して約<%= mape * 100 %>%しか乖離していないことを示しています。
  <% if (mape < 0.1) { %>
  これは非常に高い予測精度です。
  <% } else if (mape < 0.2) { %>
  これは良好な予測精度です。
...
- **R²値**: <%= r_squared %>
  <% if (r_squared > 0.8) { %>
  この値は極めて高く、モデルが<%= target %>のばらつきの<%= r_squared * 100 %>%を説明できていることを示しています。
  <% } else if (r_squared > 0.5) { %>
  この値は良好で、モデルが<%= target %>のばらつきの<%= r_squared * 100 %>%を説明できていることを示しています。
  <% } else { %>
