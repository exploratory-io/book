...
単語の組み合わせをカウントする範囲を変更したい場合、[「設定」](//analytics/settings/word_pair)から変更することができます。
...
ストップワードなどを除いた後、「隣接している単語の組み合わせ」のみをカウントします。
単語の隣接判定は、ストップワード等を除去した後に行われます。そのため、元の文章では単語が離れていても、除去後に隣接していればペアとしてカウントされます。
