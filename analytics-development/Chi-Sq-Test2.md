...
<% if (power < 0.5) { %>
検出力は<%= power_pct %>% (<%= power %>)と低い値になっています。これは、もし実際に<%= target %>と<%= explanatory %>の間に関係があったとしても、それを正しく検出できる確率が約<%= power_pct %>%しかないということです。言い換えると、**本当は関係があるのに「関係がない」と誤って結論づけてしまう確率（タイプ２エラー）が<%= type2_error_pct %>% (<%= type2_error %>)**もあるということなので、有意なのかどうかの判断が
<% } else { %>
