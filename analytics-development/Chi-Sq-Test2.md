...
<%= target %>と<%= explanatory %>のそれぞれのグループの組み合わせにおける、調整済み標準化残差をピボットテーブルで示します。列は<%= target %>のグループ、行は<%= explanatory %>のグループで、各組み合わせの残差をバーで表示します。正の値は期待値より観測値が多いこと、負の値は少ないことを表します。
...
| 条件 | 色 | 意味 |
|---|---|---|
| 残差 ≥ +3 | 濃い青 | 期待値より大幅に多い（強い差） |
| 残差 ≥ +2 | 中間の青 | 期待値より多い（中程度の差） |
| 0 < 残差 < +2 | 薄い青 | 期待値よりわずかに多い |
| -2 < 残差 < 0 | 薄い赤 | 期待値よりわずかに少ない |
| 残差 ≤ -2 | 中間の赤 | 期待値より少ない（中程度の差） |
| 残差 ≤ -3 | 濃い赤 | 期待値より大幅に少ない（強い差） |
| 残差 = 0 | グレー | 期待値と同じ（差なし） |

±2/±3の閾値は、調整済み標準化残差における一般的な有意性の目安です。

## 組み合わせの一覧

調整済みP値は、この表内のセルに対して Holm 法で補正しています。有意でないP値のみグレーで表示します。
