...
<% if (!test_mode) { %>
<%= target %> を予測するモデルが作られました。<%= period %><%= unit %>後までを予測していますが、こちらの予測期間は[「設定」](//analytics/settings/forecast_period)の「予測期間」より変更できます。
<% } else { %>
<%= target %> を予測するモデルが作られました。現在テストモードであるため、データの最後から<%= test_period %><%= unit %>前までのデータを使ってモデルを作り、それ以降の期間（テスト期間）に対して予測しています。テスト期間はアナリティクスの[「設定」](//analytics/settings/test_period)の「テストデータの期間」のセクションより変更できます。
<% } %>
...
Prophetモデルがデータの中から読み取った、毎年繰り返される<%= target %>のパターン（年周期）です。
...
Prophetモデルがデータの中から読み取った、毎四半期繰り返される<%= target %>のパターン（四半期周期）です。
...
Prophetモデルがデータの中から読み取った、毎月繰り返される<%= target %>のパターン（月周期）です。
...
Prophetモデルがデータの中から読み取った、毎週繰り返される<%= target %>のパターン（週周期）です。
...
Prophetモデルがデータの中から読み取った、毎日繰り返される<%= target %>のパターン（日周期）です。
