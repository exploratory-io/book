...
それぞれのエラーバーの真ん中の点は係数（率比、Rate Ratio）の値、上下の線はその95%信頼区間を表しています。<%= target %>との関係が有意で、かつその関係が正の予測変数は青、関係が負の予測変数は赤となっています。グレーで表されている予測変数は<%= target %>との関係が有意とは言えません。有意性の判断はP値または信頼区間によって行うことができます。どちらで判断しても同じ結果となります。
...
係数の95%信頼区間は、「真の係数の値がこの範囲内にある可能性が95%ある」ということを意味します。ポアソン回帰では、係数の指数を取った値（率比、Rate Ratio）を解釈します。率比が1であるということは、予測変数の値が変化しても<%= target %>の期待値が変わらないため、<%= target %>との関係がないことを表します。
...
それぞれの予測変数の係数はそのデータ型によって以下のように解釈できます。ポアソン回帰では、係数の指数を取った値（率比、Rate Ratio）を使って解釈します。

**数値型の場合：**

他の変数の値が一定だとしても、変数が1単位上がると、<%= target %>の期待値（予測値）は係数の指数倍になります。例えば、係数の指数が1.2であれば、変数が1単位増加すると<%= target %>は20%増加します。

**ロジカル型の場合：**

他の変数の値が一定だとしても、変数がTRUEの場合、FALSEに比べ<%= target %>の期待値（予測値）は係数の指数倍になります。例えば、係数の指数が0.8であれば、変数がTRUEのときは<%= target %>は20%減少します。

**カテゴリー（文字列）型の場合：**

他の変数の値が一定だとしても、変数のカテゴリーは、ベースレベルと比べて<%= target %>の期待値（予測値）が係数の指数倍になります。例えば、係数の指数が1.5であれば、そのカテゴリーではベースレベルに比べて<%= target %>は50%増加します。
...
* オフセット変数の検討：露出期間や観察期間が異なる場合は、オフセット変数を使用することで、単位時間あたりの発生率を正確にモデル化できます。
...
このチャートは、モデルの適合度を評価するために役立ちます。残差がランダムに分布し、特定のパターンを示していない場合は、モデルがデータに適合していることを示します。しかし、明確なパターンがある場合（例えば、予測値が大きくなるにつれて残差が大きくなる場合）は、モデルの仮定が満たされていない可能性があります。
