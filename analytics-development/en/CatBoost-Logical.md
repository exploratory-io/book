...
A CatBoost model has been created to predict <%= target %> based on the selected predictor variables.

<% if (categorical_features_count > 0) { %>
CatBoost handled <%= categorical_features_count %> categorical predictor(s) natively without one-hot encoding.
<% } %>

<% if (boosting_type_label === 'Ordered') { %>
This model uses Ordered Boosting, which reduces target leakage on small datasets at the cost of training speed.
<% } %>

<% if (task_type_label === 'GPU') { %>
This model was trained using GPU acceleration (CatBoost task_type = 'GPU').
<% } %>
...
The following chart shows the model learning process. The horizontal axis represents the number of learning iterations, and the vertical axis represents the selected evaluation metric.
Depending on the metric, the value may increase or decrease as the number of learning iterations increases. When the learning curve starts to flatten, it may be approaching the optimal number of learning iterations.
...
* Parameter Tuning: You may be able to further improve model performance by adjusting CatBoost parameters (tree depth, learning rate, number of learning iterations, etc.).
<% if (!test_mode) { %>
