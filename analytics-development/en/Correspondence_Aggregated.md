const template = `
<% var methodName = 'Correspondence Analysis'; %>
...
<% var isWideFormat = (typeof is_wide_format === 'undefined' || is_wide_format === null) ? true : !!is_wide_format; %>

# Summary

<% if (isWideFormat) { %>We ran <%= methodName %> on an aggregated cross tabulation to explore the relationships between the following two categorical variables.<% } else { %>We ran <%= methodName %> on aggregated data to explore the relationships between the following two categorical variables.<% } %>

<% if (typeof variables !== 'undefined' && variables && variables.length > 0) { variables.forEach(function(v){ %>
- <%= (typeof v === 'object' && v.name) ? v.name : v %>
<% }); } %>

Correspondence Analysis summarizes the relationships between the categories of two categorical variables into a small number of dimensions and visualizes them. By comparing the distribution pattern of each category, you can explore which categories are distinctive and which category combinations may be related.

<% if (isWideFormat) { %>The input is an already aggregated cross tabulation: the row category column holds the row categories, and each selected count column supplies one column category. The counts are used as the cross tabulation directly, so the result matches what you would get by running Correspondence Analysis on the raw data behind these counts.<% } else { %>The input is already-aggregated data with one row per row-category/column-category combination: the row category column and the column category column hold the categories, and the count column holds the count for that combination. These counts are used to build the cross tabulation directly, so the result matches what you would get by running Correspondence Analysis on the raw data behind these counts.<% } %>
...
<% if (isWideFormat) { %>The table below adds each dimension's score, computed by <%= methodName %>, to the aggregated cross tabulation you supplied. Each row is one row category, so the scores show where that row category sits on each dimension.<% } else { %>The table below adds each dimension's score, computed by <%= methodName %>, to the aggregated data you supplied. Each row is one row category, so the scores show where that row category sits on each dimension.<% } %>
