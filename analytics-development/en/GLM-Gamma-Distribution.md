...
A generalized linear model (gamma distribution) was created to predict <%= target %> based on the selected explanatory variables.
...
The coefficients show how much the value of <%= target %> changes when the value of each variable changes by 1 unit. In generalized linear models (gamma distribution), incidence rate ratios using the exponential of coefficients are used for interpretation.
...
Residual deviance is commonly used as a metric to evaluate the prediction accuracy of gamma distribution GLM prediction models.
...
* Model Comparison: It may be beneficial to compare the fit of gamma distribution and normal distribution generalized linear models to determine which is more suitable for the data. This can be judged by comparing AIC and BIC.
<% if (!test_mode) { %>
