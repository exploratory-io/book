...
結果として、P値は<%= 100 * p %>% (<%= p %>)で、有意水準<%= 100*baseline_p %>% (<%= baseline_p %>) より大きいため、<%= explanatory %>による<%= target %>の平均の差は統計的に有意とは言えません。
  ...
  <% if (power < 0.5) { %>
ただ、検出力は<%= 100 * power %>% (<%= power %>)と低く、もし実際に差があったとしても、見逃す確率（タイプ２エラー）が<%= 100 - 100 * power %>% (<%= 1 - power %>)と非常に高いため注意が必要です。
  <% } else { %>
ただ、検出力は<%= 100 * power %>% (<%= power %>)と低めであり、もし実際に差があったとしても、見逃す確率（タイプ２エラー）が<%= 100 - 100 * power %>% (<%= 1 - power %>)もあるため注意が必要です。
  <% } %>
<% } %>
...
<% if (p > baseline_p) { %>
検定の結果、P値が<%= 100 * p %>% (<%= p %>)となりました。これは、もし帰無仮説が正しいのであれば、今回のデータのように<%= mean_diff %>以上の差がたまたま起きる確率が約<%= 100 * p %>%>であるということです。有意水準が<%= 100*baseline_p %>% (<%= baseline_p %>) の場合、この確率は十分に高いので、帰無仮説を棄却できません。つまり、<%= explanatory %>による<%= target %>の差は統計的に有意であると言えません。
<% } %>
