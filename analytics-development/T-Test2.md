...
<% if (p <= 0.05) { %>
  ...
  <% if (effect_size < 0.2) { %>
    効果量は<%= 100 * effect_size %>と低く、<%= explanatory %>による<%= target %>の平均の差は小さなものです。
  <% } else if (effect_size < 0.8) { %>
    効果量は<%= 100 * effect_size %>なので、<%= explanatory %>による<%= target %>の平均の差は中程度です。
  <% } else { %>
    効果量も<%= 100 * effect_size %>と大きく、<%= explanatory %>による<%= target %>の平均の差は大きいと言えます。
  <% } %>
<% } %>
...
この検定では効果量であるCohen's Dが<%= cohens_d %>と示されています。これは、<%= explanatory %>による<%= target %>の平均値の違いの大きさを標準化したものです。
...
<% if (power < 0.5) { %>
  検出力は<%= 100 * power %>% (<%= power %>)と低い値になっています。これは、もし実際に<%= explanatory %>によって<%= target %>の平均に差があったとしても、それを正しく検出できる確率が約<%= 100 * power %>%>しかないということです。言い換えると、**本当は差があるのに「差がない」と誤って結論づけてしまう確率（タイプ２エラー）が<%= 100 - 100 * power %>% (<%= 1 - power %>)**もあるということなので、有意なのかどうかの判断が困難となります。
<% } else if (power < 0.8) { %>
  検出力は<%= 100 * power %>% (<%= power %>)とやや低めになっています。これは、もし実際に<%= explanatory %>によって<%= target %>の平均に差があったとしても、それを正しく検出できる確率が約<%= 100 * power %>%>しかないということです。言い換えると、**本当は差があるのに「差がない」と誤って結論づけてしまう確率（タイプ２エラー）が<%= 100 - 100 * power %>%**もあるということなので、慎重な判断が必要です。
<% } else { %>
