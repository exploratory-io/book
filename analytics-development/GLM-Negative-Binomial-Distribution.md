...
係数の95%信頼区間は、「真の係数の値（発生率比）がこの範囲内にある可能性が95%ある」ということを意味します。そのため、95%信頼区間が1を含んでいる予測変数は、<%= target %>との関係が全くない可能性があるため、統計的に有意だと言えません。逆に、95%信頼区間が1を含んでいない予測変数は、<%= target %>との関係が全くない可能性はほぼないため、統計的に有意だと言えます。
