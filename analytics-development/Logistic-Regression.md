...
それぞれの予測変数の値が変わると、<%= target %>の値がどのように変わるかを表したのが以下のチャートです。
...
それぞれのエラーバーの真ん中の点は係数（オッズ比）の値、上下の線はその95%信頼区間を表しています。<%= target %>との関係が有意で、かつその関係が正の予測変数は青、関係が負の予測変数は赤となっています。グレーで表されている予測変数は<%= target %>との関係が有意とは言えません。
