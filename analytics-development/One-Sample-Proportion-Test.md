...
以下のチャートは標準正規分布を表しています。点線は標準化された検定統計量（Z 値）を示し、網掛け部分は有意水準における棄却域です。Z 値が網掛けの領域に入る場合、ベンチマークとの差は統計的に有意です。

{{probability_dist}}

以下のチャートは、同じ検定を比率のスケールで表したものです。比率がベンチマークと等しいと仮定したときの標本比率の正規分布（中心はベンチマークの比率）で、標準偏差は標準誤差に等しくなります。点線は観測された比率を示し、網掛け部分は有意水準における棄却域です。観測された比率が網掛けの領域に入る場合、ベンチマークとの差は統計的に有意です。
