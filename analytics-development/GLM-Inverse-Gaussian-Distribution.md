...
このモデルの情報量規準（AIC: <%= aic %>, BIC: <%= bic %>）は、モデルの複雑さと適合度のバランスを評価する指標です。値が小さいほど優れたモデルとされ、異なるモデル間の比較に使用できます。
...
* 他の分布の検討：逆ガウス分布は非負の連続データで右に歪んだ分布のモデル化に適していますが、データの特性によっては他の分布（ガンマ分布など）も検討する価値があります。
