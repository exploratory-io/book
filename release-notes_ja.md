...
- プロキシサーバーは、Rの連携に必要なソフトウェアをダウンロードして、自動的な認証をできるようになりました。詳しくは[FAQ](https://exploratory.io/faq) をご覧ください。ユーザー名とパスワードが必要なときは、ユーザーは、configurationファイルに加えることができます。
- CSV / DelimitedFilesをインポートするときに、１０や１０００のマークの新しいパラメータの設定といっしょに、アメリカ式ではない数字データもサポートしました。

### バグ修正

- アプリケーションを最初に起動したときに、真っ白のスクリーンのまま動かなかったり、UIをローディングしたまま動かなかった問題を修正しました。
- ホームディレクトリが空白を持って行く時に、プロジェクトを開くことができなかったのを修正しました。
- チャート画面で、X軸、Y軸、Colorにアスキー文字以外の文字を指定できなかったのを修正しました。。
- チャート画面で、列がNA値を持っているX軸に指定されたときBoxplotが、うまく作動しなかったのを修正しました。
