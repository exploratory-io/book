...
- データと分析ステップと一緒にチャートを公開できるようになりました。これで、他の人がどうやってそのチャートを作ったのかが理解しやすくなりました。
- 毎回、コネクション情報を入力する代わりに、データベースにアクセスしたときに情報を選ぶことができるように一連のコネクションパラメータを持った'connections'を作ることができるようになりました。
- データ分析のステップから関数を呼んでこれるように、自分のプロジェクトのために、R関数を定義したRスクリプト作ることができるようになりました。
- プロジェクトごとに読み込むRのパッケージを選ぶことができるようになりました。これは、変数を定義するときに使うこともできます。
- SQLのクエリの一部として、Rスクリプトで定義されている自分の変数を使うことができるようになりました。

### Enhancements

- ヒストグラムのチャートを作るときに、データのバケツの数を変更できるようになりました。
- Google AnalyticsのPage-nationを10K行以上引き出すことができるようになりました。
