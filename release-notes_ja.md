...
* AI: AIプロンプト: @を使ってデータフレームを参照できることを説明するテキストをダイアログに追加しました。
* AI: AI関数: AI関数をすぐに使い始められるサンプル関数テンプレートを追加しました。
* AI: AI関数: パラレルオプションが有効でない場合に最大行数の上限が3,000であることを表示するようにしました。
* AI: AI関数: 10,000行制限での処理前にNA値を除外する機能を追加しました。
* AI: AIプロンプト: AIが生成するステップで数値計算関数をより適切に使用するよう改善しました。

* データソース: 廃止されたMailChimp接続を削除しました。
* データソース: CSV: インポートする最大行数を指定するオプション（n_max）を追加しました。
* データソース: GitHub: 10K制限を超える過去のイシューを取得するためのフィルターパラメーターのサポートを追加しました。
* データソース: データベース: ジョブウィンドウのRServeステータスが無効になった際にRServeを自動再起動するようにしました。
* データソース: Excel: 無効なファイル形式の検出とユーザーフレンドリーなエラーメッセージを追加しました。
* データソース: GitHub: 日付範囲やその他のフィルターパラメーターのサポートを追加しました。

* データラングリング: テーブルビュー: ホバーポップアップでコピーボタンをクリックした際に「コピーしました！」のフィードバックを追加しました。

* チャート: 集計テーブル: グループごとに最頻値を求めて保存する機能を追加しました。
* チャート: ナンバー: サフィックス/プレフィックスのテキストに対する部分的なスタイルカスタマイズをサポートしました。
* チャート: テーブル: テーブル、ピボットテーブル、集計テーブルでホバーポップアップのコピーフィードバックとバルーンを改善しました。
* チャート: フィルター: チャートフィルター実行後にフィルターリストダイアログが自動的に閉じるようになりました。
* チャート: 表示するデータがない場合にチャートデータと画像のエクスポートを無効化しました。
* チャート: フィルター:「日（月内）」の値タイプに「範囲指定（Between）」演算子のサポートを追加しました。
* チャート: 散布図: 日付カラムを軸に割り当てた際に軸幅として期間を指定できるようにしました。
* チャート: バー: 0の値を表示するサポートを追加しました。
* チャート: ピボットテーブル: フォントサイズに基づいて行の高さを自動調整するサポートを追加しました。
* チャート: Y軸メニューのマーカーにショートカットメニューを追加しました。
* チャート: ピボットテーブル: 垂直方向の罫線（グリッドライン）の表示をサポートしました。

* アナリティクス: アナリティクスビューにLightGBMメニューを追加しました。
* アナリティクス: 因子分析: 因子スコアの説明を追加しました。
* アナリティクス: クラスタリング: パラレルコーディネートのプロパティセクションを削除しました。
* アナリティクス: LightGBMのサポートを追加しました。

* ダッシュボード: 編集モードでチャートをダブルクリックして行を選択できるようにしました。
* ダッシュボード: 背景色の変更をサポートしました。
* ダッシュボード: ナンバー: フォントサイズの設定をサポートしました。

* ノート: 目次（TOC）のスタイルを改善しました。
* ノート: UIのロケールが日本語の場合、PDF印刷ダイアログが日本語で表示されるようになりました。

* パラメーター: シングルセレクトLOV: ラベルを更新しフィールドの位置を調整しました。

* 一般: 古いログファイルの自動削除を追加しました。
* 一般: シャットダウン時のクリーンアップ処理を改善しました。
* 一般: RServeが接続拒否エラーでクラッシュした際に自動再起動するようにしました。

* インストール: Mac: プロジェクトフォルダ作成エラーを防ぐためにリポジトリフォルダの書き込み権限チェックを追加しました。

### 修正された問題

* AI: AI関数: データフレームを複製した際に、キャッシュされたステップが再実行されていました。
* AI: AIプロンプト: 外れ値の除去機能が動作しませんでした。
* AI: データラングリング: AI指示内の全角記号が生成されたRスクリプト内で自動的に半角記号に変換されていました。
* AI: AIプロンプト: 複数行を入力するとテキストがエディタエリアからはみ出していました。
* AI: AI関数: AI関数の実行時にHTTPリクエストエラーが発生していました。
* AI: AIプロンプト: AIプロンプトでhms（時間）型が正しく変換されませんでした。

* データソース: クラウドストレージ: カラム数の上限を高く設定しても、CSVインポートのプレビューで100列目以降が表示されませんでした。
* データソース: リモートCSV: データソースタイプを変更した際にファイルパスとプレビューが更新されませんでした。
* データソース: Rスクリプト: インポートダイアログでプレビューデータが保持されませんでした。
* データソース: GitHub: 全てのカラムの選択を解除してから一部を選択し直すと、不要なカラムがインポートされていました。
* データソース: Googleスプレッドシート: データインポート時にデータ型の変換が正しく動作していませんでした。
* データソース: インポート: 条件付きインポートで濁点を含む文字列を正しく処理できませんでした。
* データソース: インポート: yyyy/m/d形式の日付がデータ型変換時に欠損値になっていました。
* データソース: Excel: Excelファイルに複数のシートがあり最初のシートが空の場合、インポートダイアログが横スクロールできませんでした。
* データソース:「データ型を自動変換する」チェックボックスをオフにしてもデータ型が変換されていました。

* データラングリング: テーブルビュー: 一番下までスクロールした後にマウスホイールを使うと先頭に戻ってしまっていました。
* データラングリング:「関数nullが見つかりません」というエラーが発生していました。
* データラングリング: ジョブ（SQLインポートとパブリッシュ）のキャンセルに長い時間がかかっていました。
* データラングリング: トークンをクリックした際にShiftキーでの複数ステップ選択が動作しませんでした。
* データラングリング: プロセスのキャンセルに長い時間がかかっていました。

* チャート: バー: Y軸の値を複数選択した際に、X軸のリファレンスラインのラベルが重複して表示されていました。
* チャート: テーブル: 大きなテーブルを持つチャートの保存が非常に遅くなっていました。
* チャート: テーブル: テキストが長い場合にポップアップが切れていました。
* チャート: Y軸マーカーのサブメニューとカラムリストの垂直方向の位置がずれていました。
* チャート: X軸で「値の制限」を適用すると、色カラムのレベルが消えていました。
* チャート: エリア: リピートバイを使用した際にX軸の日付スケールがずれていました。
* チャート: ピボットテーブル: カラムをソートした際に合計カラムがソート順に従いませんでした。
* チャート: 散布図: Y軸の小数点桁数を設定するとデータポイントのクリックで詳細データが表示されませんでした。
* チャート: ピボットテーブル:「uniteに適用可能なメソッドがない」エラーでレンダリングに失敗していました。
* チャート: ピボットテーブル:「returnsNumericValueに無効な引数が渡されました」エラーでレンダリングに失敗していました。
* チャート: 論理カラムのパーセンテージの詳細データを表示する際に、TRUEまたはFALSE以外の値が表示されていました。
* チャート: ピボットテーブル: ピボットテーブルが全行をレンダリングしていない場合、データエクスポートに全行が含まれませんでした。
* チャート: 値の表示: ポジションのデフォルトが「自動」に設定されていませんでした。
* チャート: 値の表示: しきい値を0に設定しても矢印を表示できませんでした。
* チャート: 値の表示: 自動配置でNA値のテキストが間違った位置に表示されていました。
* チャート: フィルター/リファレンスライン: 無効にした条件がプレビューに表示されたままでした。
* チャート: 散布図: Y軸ラベルの小数点を0に変更すると詳細表示が正しく動作しませんでした。
* チャート: ピボットテーブル: ExcelやクリップボードにエクスポートするときTotalsとSubtotalsが含まれていませんでした。
* チャート: 集計テーブル: CSVにエクスポートするとグループに割り当てた値が最初の行以降で欠損値になっていました。
* チャート: エリア: 色で可視化した際に値が誤って積み上げられていました。
* チャート: テーブル: スクロールバーをドラッグして下にスクロールした後にマウスホイールを使うとスクロール位置がリセットされていました。
* チャート: パイ: リピートバイを使用した際に各パイにタイトルが表示されませんでした。

* アナリティクス: 相関: 相関アナリティクスでテーブルが表示されませんでした。

* ダッシュボード: PDF出力: ナンバーのサブメトリクスのフォーマットが正しくなく、ピボットテーブルの色フォーマットが反映されず、マップが表示されませんでした。

* ノート: メトリクス付きのナンバーチャートがWord出力で間違ったメインの値を表示していました。
* ノート: 検索ウィジェットで文字を入力するとすぐにフォーカスが外れていました。
* ノート: 2カラムレイアウト内に配置したナンバーチャートが完全に表示されませんでした。
* ノート: マークダウンをノートに貼り付けた際に、絶対パスの画像が表示されませんでした。
* ノート: 2カラムレイアウトで右側が消えていました。
* ノート: マップチャートの画像エクスポートが「チャート画像データが空」エラーで失敗していました。
* ノート: マークダウンテキストをコピー＆ペーストした後、正しくフォーマットされませんでした。
* ノート: Rコードブロック: プレビュータブ内のテキストを選択できませんでした。
* ノート: Rスクリプトブロック: Rスクリプトブロックの実行後、出力もエラーも表示されない場合がありました。
* ノート: 特定のノートでエディタ画面が空白になり応答しなくなっていました。
* ノート: ノートを開いた際に、データがあるにもかかわらずチャートが「表示するデータがありません」と表示されていました。
* ノート: ナンバーチャートを含むノートをWordファイルにエクスポートすると失敗していました。
* ノート: chordDiagramの出力が返されず、プレビューボタンをクリックするとエラーダイアログが表示されていました。
* ノート: サンキーダイアグラムがプレビューおよびパブリッシュされたノート/ダッシュボードで空白になっていました。

* パラメーター: セッションパラメーターに依存するパラメーターが正しい結果を返しませんでした。
* パラメーター: デフォルト値がデータフレームとして参照される値と一致しませんでした。
* パラメーター: 今日以降の日付のパラメーターを実行するとプロセスが完了しませんでした。

* 一般: オフラインモードがExploratory再起動後に保持されませんでした。
* 一般: データフレーム名にカーソルを合わせるとポップアップがデータフレームメニューと重なっていました。
* 一般: Excel出力で「コンテンツに問題」エラーが発生し修復ダイアログが必要になっていました。
* 一般: セキュリティ設定で許可されているにもかかわらず、Intercomチャットで.logファイルを添付できませんでした。
* 一般: プロジェクトを開く際に、パッケージが削除されていないにもかかわらずカスタムパッケージが見つからない警告が表示されていました。
* 一般: EDF/EPFからインポートしたデータベース接続でコンテンツをパブリッシュすると接続が見つからないエラーが発生していました。
* 一般: 履歴からプロジェクトを復元すると「index.lockファイルを作成できません」エラーで失敗していました。
