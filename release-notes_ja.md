...
* データソース: Google Sheet: インポート時に列のデータ型を上書きできるようになりました。
  

* データラングリング: ホバーポップアップでテキストをコピーできるようになりました。
  

* チャート: テーブル / ピボットテーブル のホバーポップアップでテキストをコピーできるようになりました。
* チャート: 「8/1」のように十の位にゼロが付かない日付フォーマットがサポートされました。
* チャート: フィルターおよびリファレンスライン用にボタンスタイルのメニューが追加されました。
...
* チャート: リファレンスライン: 累積合計値 / 累積合計値の割合 (%) において、すべてのチャートタイプで Y2 軸が利用できるようになりました。
* チャート: リファレンスライン: 累積合計値 / 累積合計値の割合 (%) および移動平均でグループ化オプションがサポートされました。
...
* AI プロンプト: オフライン時に AI データ加工 ボタンが表示されませんでした。


* データソース: データベース: ウィンドウを最大化すると、編集中の SQL が消えていました。
* データソース: インポートボタンをクリックした後、ファイル選択ボタンが表示されませんでした。
* データソース: Google Analytics: "Cannot read properties of undefined (reading 'some')" のエラーが発生していました。
* データソース: インポート: SQLクエリのキャンセル後、インポートダイアログの「キャンセル」ボタンが反応しませんでした。


* データラングリング: 条件を元に値を置き換える: 既存の条件設定が「作成中...」と表示されてしまう問題を修正しました。
* データラングリング: フィルター: 「この文字で始まる」内の「複数の値を指定」が機能しませんでした。
* データラングリング: データフレームに外部データフレームが関連付けられている場合、どのステップからでもデータインポートダイアログが表示されるべきでした。
* データラングリング: データ再インポートダイアログが前回の選択を記憶していませんでした。
* データラングリング: データフレームを削除しても残ることがある問題を修正しました。
* データラングリング: 再インポートダイアログで、ブランチのブランチに参照されるデータフレームが一覧に表示されませんでした。
* データラングリング: 「この文字で始まる」において複数値を指定した場合、条件式が空になっていました。
* データラングリング: EDF ファイルをインポートした際に、不要な「imp」接尾文字が付く問題を修正しました。


* チャート: チャートがピン留めされていないケースがありました。
* チャート: ピボットテーブル / 集計テーブル: 「% of」のウィンドウ計算適用時に追加の合計が正しく機能しませんでした。
* チャート: ピボットテーブル: カスタム計算値が誤ったカラーパレットを使用していました。
* チャート: ピボットテーブル: 「表示する値の制限」を使用時にエラーが発生しました。
* チャート: ピボットテーブル: 値が切り捨てられた場合、ポップアップが出たり出なかったり不安定でした。
* チャート: ピボットテーブル: 複数値選択時、最後の値に対する「小計のみ表示」「合計のみ表示」が機能しませんでした。
* チャート: リファレンスライン: 累積合計 / 累積比率および移動平均の計算が散布図・バブルチャートで正しくありませんでした。
* チャート: リファレンスライン: Y に移動平均、X と Y に特定値を追加するとエラーが発生しました。
* チャート: リファレンスライン: 2本目のラインのラベルが「凡例に表示」をオンにしても表示されませんでした。
* チャート: リファレンスライン: ロジカル型のデータタイプの列の時に Y 軸スケールが正しくありませんでした。
* チャート: リファレンスライン: 累積合計値の割合のリファレンスライン設定時、Y2 軸タイトルとラベルが重なり、ラベルが切れていました。
* チャート: 散布図: X 軸に日付列を割り当てた場合、リファレンスラインが表示されませんでした。
* チャート: 散布図: Y 軸をパーセント形式にすると詳細データが表示されませんでした。
* チャート: 箱ひげ図: X と色に同じ列を割り当てた際、ソートが機能しませんでした。
* チャート: テーブル / ピボット / 集計テーブル: カスタム計算を使った場合、ユーザーパレットを選択しても常にグレーのパレットを返しました。

* アナリティクス: ガイド付きアナリティクスで P 値の小数表示桁数が正しく変更されませんでした。
* アナリティクス: 決定木: 値が 0.5 のノードで背景の灰色が表示されませんでした。
* アナリティクス: アナリティクスビューでパラメーター変更後に処理が終了せず、ビジー表示が消えませんでした。
* アナリティクス: 決定木で日本語を含む値の列を選択すると凡例が文字化けしました。
* アナリティクス: 教師あり学習モデル: 予測子として日付列を使用すると、日の値が誤ってカテゴリとして扱われました。
* アナリティクス: t検定: 等分散を仮定するメニュー名を変更しました。
* アナリティクス: t検定（集計データ）: データ分布タブの読み込みが完了しませんでした。
* アナリティクス: ワードカウント: 共起ネットワークのサイズ設定を変更すると、別のデータフレームの結果が表示されました。


* ノート: フルサイズ画像の表示位置が正しくありませんでした。
* ノート: EDF を重複したデータフレームを含むプロジェクトにインポートすると、重複したデータフレームに紐づくチャートがインポートされませんでした。
* ノート: EDF ファイルのインポートすると、インポートされたノートの名前に不要な「_1」が追加されました。
* ノート: ワードクラウドチャートが編集モードで壊れたアイコンとして表示されました。


* ダッシュボード: パブリッシュ通知メールのサムネイルにホバーポップアップが表示されていました。
* ダッシュボード: 「全ページに適用」をオフにしても、Rコードブロックがないページでパラメーター更新時にページ全体がリフレッシュされました。
* ダッシュボード: 実行せずにパブリッシュした場合、サムネイルが現在のページではなく前回実行時の最終ページを表示していました。
* ダッシュボード: ダッシュボードを複製して実行せずにパブリッシュすると、1ページ目の最初のオブジェクトがサムネイルになっていました。


* パブリッシュ: データのパブリッシュ時に “paste(csv_content...) result would exceed 2^31-1 bytes” エラーが発生しました。


* プロジェクト: 重複: 無効なデータフレームメタデータを含むプロジェクトを複製するとエラーが発生しました。


* サーバー: ヒートマップ: パブリッシュ後、デスクトップとサーバーでカラーパレットが一致しない問題を修正しました。
* サーバー: ダッシュボード: パラメーター: 同じパラメーター値でも異なる結果が返る問題を修正しました。


* 一般: Ask AI: スピンアイコンが表示されたままでした。
* 一般: プロジェクトウィンドウが1回のクリックで閉じませんでした。
