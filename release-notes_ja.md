...
* データソース: Redshift、MariaDB / MySQL、およびPostgreSQLの日本語のヘルプリンクを追加しました。
* データソース: Google BigQuery: インポートダイアログでページサイズのパラメーターを空白のままにすると、アサーションエラーが発生していました。
* データソース: Excelファイルのインポート&マージの機能で、同じシート名を持たない時のエラーメッセージを改善しました。


* データラングリング: 「重複する行を除く」の動作に関する説明を追加しました。
* データラングリング: as_Date関数が計算エディタで強調表示されませんでした。
* データラングリング: 列を選択した後に欠損値（NA）の削除のダイアログが閉じませんでした。
* データラングリング: フィルター: 日付型の列のフィルタに不要なタイムゾーンのオプションが表示されていました。
* データラングリング: 列ヘッダメニューから「全ての列が欠損値（NA）の行を除く」を実行すると、いずれかの列に欠損値が含まれていると、誤って行が削除されていました。


* チャート: 散布図（集計あり）: Y軸にPOSIXct型の列を選び「時間の抽出」を指定した後に並び替えを有効にすると、エラーダイアログが表示されました。
* チャート: 色に数値列を割り当てて表示名と色を変更した場合、色の割り当てが正しくありませんでした。
* チャート: 複数のY軸の列にカスタムの表示名を設定した場合、色の変更が適用されませんでした。
* チャート: ピボットテーブルと集計テーブルのTRUEの数、FALSEの数、欠損値の数、非欠損値の数の合計の集計関数のデフォルトを「合計」に変更しました。
* チャート: 地図 - 標準: 「世界の国（太平洋中心）」では、タイプを「サークル」にした時に太平洋の中心になりませんでした。


* アナリティクス: 時系列異常検知をステップとして作成する場合、「欠損値の処理」のパラメーターを変更できませんでした。


* 一般: Windows版のExploratory v6.9.5は、Rserveがメモリ不足で不安定でした。
* 一般: Exploratoryデスクトップを起動した時に、"The term 'dpapibridge' is not recognized as the name of a cmdlet..."のエラーが表示され、ログインプロセスが終了しませんでした。
