...
* データソース: コネクションを切り換えるとスキーマとテーブルが入れ替わってしまう。
...
* チャート: ナンバー: 比較する値がない場合、フィールドを空白スママにするのではなく「-」を表示するようになります。
* チャート: ピボット: 集計テーブル: 表計算の「差」や「差の割合」では、「絶対値として計算する」オプションをサポートする必要があります。
