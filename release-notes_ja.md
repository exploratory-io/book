...
* データソース: インポート＆マージ: 「含まない（does not contain）」演算子をサポートしました。
...
* チャート: 値の表示（Show Value）において、文字が重ならないよう自動的に位置を調整するようになりました。
* チャート: 折れ線チャート: リファレンスラインの「プロット領域にテキストを表示」で、文字が重ならないようにしました。
...
* アナリティクス: ワードカウント: 「組み合わせをカウント」のデフォルト値を「ドキュメント内」に変更しました。
...
* パブリッシュ: 社内ネットワーク環境からノートのパブリッシュに失敗する問題を修正しました。
