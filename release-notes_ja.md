...
* データラングリング: 入力フィールドのラベルを「ターゲット列」から「ターゲット列（複数可）」に変更しました。
...
* データラングリング: ケースウェン: 条件が数式ではなく論理ベクターとして指定された場合のエラーメッセージを改善しました。
* データラングリング: If Else: if_else() に予期しない引数が渡された場合のエラーメッセージを改善しました。
* データラングリング: 複数列を選択できることをより明確に示すため、入力フィールドのラベルを「対象カラム」から「対象カラム（複数可）」に更新しました。

* チャート: 参照線: 無効化された参照線を左のツールメニューでグレーアウト表示するようにし、視認性を向上させました。
* チャート: 面グラフ・棒グラフ:「積み上げ」と「重ねる」レイアウトモードの日本語ラベルを更新しました。
* チャート: テーブルビュー: 列のサマリーポップアップに「チャート」ボタンを追加し、選択した列からすばやくチャートを作成できるようにしました。

* アナリティクス: ランダムフォレスト: 結果テンプレートにROC曲線のセクションを追加しました。


* 一般: Markdown、サマリーCSV、サマリーExcelのファイルエクスポート操作に対してフォールバックエラー処理を追加しました。

### 修正された問題

* AI: AIプロンプト: 入力フィールドと実行ボタンの表示位置がずれていた問題を修正しました。

* チャート: 参照線が設定されていないチャートに予期しないX軸参照線が表示されていた問題を修正しました。
* チャート: 文字型と数値型の列間の型不一致により dplyr::bind_rows が失敗していたエラーを修正しました。
* チャート: dataframe_check でカラムまたはオブジェクトが存在しないと報告されていたエラーを修正しました。

* アナリティクス: ランダムフォレスト: 学習データにターゲット変数のクラスが1つしかない場合に train_model が失敗していたエラーを修正しました。

* アナリティクス: XGBoost: M1 MacでXGBoostのテスト結果が一致しないことがありました。
* 一般: パブリックプランでExploratoryを開こうとするとエラーが発生していました。
...
* データラングリング: テーブルビュー: ホバーポップアップのコピーボタンをクリックした際に「Copied!」のフィードバックを表示するようにしました。


* チャート: 集計テーブル:「その他のグループ」オプションを使って各グループの最頻値（モード）を計算できるようになりました。
* チャート: ナンバー: プレフィックスとサフィックスのテキストに異なるフォントスタイルを個別に適用できるようになりました。
* チャート: テーブル / ピボットテーブル / 集計テーブル: ホバーポップアップのコピーボタンをクリックした際に「Copied!」のフィードバックを表示するようにしました。
...
* アナリティクス: LightGBM がアナリティクスメニューから利用できるようになりました。
...
* ダッシュボード: ナンバーチャート: フォントサイズの設定をサポートしました。
...
* チャート: エリア: 色で分類している場合に値が誤って積み上げられていました。
