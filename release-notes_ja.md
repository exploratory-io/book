...
* データソース: Google Cloud Storage: バケット内に多数のファイルがある場合に、UI 上で http_400 エラーが発生して一覧表示できず、ファイルをインポートできないことがありました。


* チャート: 特定の条件下で、チャートの並び順が変わってしまうことがありました。
* チャート: 折れ線: ホバーモードで同じ X 軸上にすべての値を表示している際、欠損値があると次の値が正しく表示されないことがありました。


* プロジェクト: コラボレーション: プロジェクトを閉じずにアプリを終了した場合、編集中を示す緑色のランプが点灯したままになることがありました。
* プロジェクト: Exploratory 14.1 へアップグレード後に、"invalid multibyte character in parser (C:\Program Files\Exploratory\lib\library.r:4591:61" というエラーが発生して、プロジェクトを開けないことがありました。
