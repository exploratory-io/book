...
* データソース: コネクションを切り換えると、スキーマとテーブルが入れ替わってしまう。


* データラングリング: ステップの実行時に前回のインポート日時が誤って更新される。


* チャート: ナンバー: パラメーター設定を適用した後にデータがない場合、ナンバーが正しくない高さに表示される。
* チャート: ナンバー: 比較する値がない場合、フィールドを空白として表示するのではなく「-」を表示するようになります。
* チャート: ピボット: 集計テーブル: 表計算の「差」や「差の割合」では、「絶対値として計算する」のオプションをサポートする必要があります。
* チャート: ピボット: 集計テーブル: 行に2つの列を追加し、1番目の列を「隠す」機能を使って非表示にした際に、1番目の列を削除すると「隠す」のオプションがその後に列に引き継がれてしまう。


* ノート: テキスト量が多いノートに入力する場合、パフォーマンスが少し遅くなってしまう。


* パラメーター: 参照されたステップを削除した後、日付のパラメーターの列の割り当てが無効になる。


* プロジェクト: 以前のプロジェクトにパブリッシュに失敗し、保留中の変更があったプロジェクトを閉じることができない。
...
* チャート: カテゴリーを作成: 色、グループ、並び順の設定からDate型またはPOSIXct型の列を手動でグループに分けられるようになりました。
* チャート:色で分割: 集計関数を使って、色/凡例の並び順を指定できるようになりました。
* チャート: X軸の「表示する値の制限」の中で「相対日付」をサポートして直近N年/月/週/日で値を制限できるようになりました。
* チャート: 「繰り返し」の並び順を変更できるようになりました。
* チャート: ナンバー: メインの指標に加えてサブの指標を表示できるようになりました。
* チャート: ナンバー: 最後の期間の値をサブの指標として表示できるようになりました。
* チャート: テーブル: 常に行数を表示するようになりました。
* チャート: ピボットテーブル: 列の値の並び順を指定できるようになりました。
* チャート: ピボットテーブル: 集計テーブル: 行の値の並び順を指定できるようになりました。
* チャート: ピボットテーブル: 集計テーブル: 列のヘッダーのテキストを折り返せるようになりました。
* チャート: ピボットテーブル: 集計テーブル: 列のヘッダーのテキストが長いときに、全てのテキストを表示できるように、列の幅のメニューで「ヘッダーのテキストに合わせる」オプションをサポートしました。
...
* ノート: デフォルトの幅で高さだけが高いチャートの表示サイズをサポートしました。
* ノート: ノート内のテキストの検索と置換をサポートしました。


* パラメーター: ステップで列を動的に選択できるように、quoteオプションをサポートしました。


* プロジェクト: プロジェクト・オブジェクト・ツリーからデータフレームやレポートを名前順で並び替えたときに、数字で始まるデータフレームやレポートは、数値をもとに並び替えられるようになりました。
* プロジェクト: チームで共有されているプロジェクトに、アカデミックプランのユーザーを招待できるようになりました。


* 一般: テーブルビューにおける数値の桁区切り文字と小数点の数値フォーマットを変更できるようになりました。
