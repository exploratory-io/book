...
*   データフレーム名、ステップ名、ノート名、ダッシュボードのページ名を変更しようとすると「a.getMarkdownContent is not a function」というエラーが発生していました。


### アナリティクス

*   アナリティクスを実行した後も、古いデータの警告メッセージが消えませんでした。
...
*   コメントアイコンが、作成者のコメントだけでなく、閲覧者のコメントが存在する場合にも緑色に変わるようになりました。



### AIアナリティクス

*   トピックモデルのデータがすでに単語化されている場合の、AIサマリーでの生データ表示を改善しました。
...
*   クリーンインストール後の初回実行時に、AIデータ加工ダイアログが空白の結果を返していました。

### AI関数

*   AI関数の実行をキャンセルした後に再実行するとエラーが発生していました。

### アナリティクス

*   対応ありのウィルコクソン検定の結果がRの結果と異なっていました。

### 一般

*   AIサマリーを実行後にデータフレームを切り替えると、メモリリークにより動作が遅くなっていました。
...
*   サマリーチャート上に「チャート」ボタンが追加され、テーブル・ビューからでも素早くチャートが作れるようになりました。

### チャート

*   エリアチャートやバーチャートにおいて、「積み重ね」と「オーバーレイ」の日本語ラベルがより適切な表現に更新されました。
*   チャートの画像エクスポート失敗時に、フォルダ権限の確認や再試行を行い、状況に応じた適切なエラーを表示するよう改善されました。
*   無効化した基準線がツールメニュー上で有効なものと区別しにくかったため、視覚的にグレーアウトされるようになりました。

### ノート

*   AIノートエディターの実行中に、本文の画面のちらつきを抑えてユーザー体験を向上させました。

### ドキュメント

*   正規表現のヘルプボタンのリンク先が、より適切で最新の解説ノートへと更新されました。
...
*   AIプロンプトの入力フィールドと実行ボタンの表示位置がずれ、他のテキストと重なってしまう不具合がありました。

### チャート

*   ピボットテーブルで合計行を表示してCSVエクスポートする際、特定の列で異なるデータ型が混在するとエラーになる問題がありました。
*   ピン留めされたチャートが、無効化された前のステップに依存している場合に、キャッシュ保存やレンダリングでエラーが発生していました。
*   マーカーのショートカットメニュー導入後、マーカーのスタイル設定メニューにアクセスできなくなっていました。
*   設定していないはずのX軸基準線がチャートに表示されたり、古い設定データが残存して意図しない表示になる不具合がありました。

### テーブル・ビュー

*   ピボットテーブルの値にNAが含まれるデータをCSVエクスポートする際、データ型の不一致でエラーになる問題が修正されました。
*   表計算でCharacter型の列をグループに、POSIXct型の列を値に設定した際、UI上で値フィールドが未選択に見える不具合がありました。

### ノート

*   AIノートエディタのダイアログを上に移動した際、下側の境界線が表示されない問題がありました。
*   TOC（目次）が長すぎる場合に、スクロールして残りの項目を表示することができませんでした。

### パラメーター

*   Between（範囲）演算子でパラメータを選択した際、ドロップダウンが2つ表示されたりUIが崩れたりする問題がありました。
...
*   機械学習モデルとしてLightGBMをUIから直接利用できるようになりました。

## 機能強化

### AI プロンプト

*   AIプロンプトで数値計算を指示した際、存在しない関数が使われないよう、適切な数値計算関数を使用するように改善されました。
*   AIプロンプトのダイアログに、結合先のデータフレームを「@」で指定できる旨の説明テキストが追加されました。

### AI 関数

*   AI関数のメニューに、テキスト要約やセンチメント分析、翻訳などのよく使われるタスクの例が追加されました。
*   AI関数の処理において、1万行の制限を適用する前に欠損値を除外して有効なデータを優先的に処理できるようになりました。
*   AI関数を実行する際、並列処理がオフで3000行を超える場合に、処理上限に関する確認ダイアログが表示されるようになりました。
*   データフレームを複製した際に、キャッシュされたAI関数のステップが不要に再実行されていた問題が改善されました。

### チャート

*   Y軸メニューのマーカー設定にショートカットメニューが追加され、利用可能なタイプを素早く選択できるようになりました。
*   エリアチャートを色で分割した際に、値を積み上げずにY軸の0から開始して可視化できるようになりました。
*   チャートに表示するデータがない場合、エラーを防ぐためにイメージやデータのエクスポートボタンが無効化されるようになりました。
*   チャートの「値を表示」設定において、表示位置のデフォルトが「自動」に設定されました。
*   チャートのテーブルやピボットテーブルのホバーポップアップにおいて、コピー時のフィードバック表示やバルーンサイズの拡大などの改善が行われました。
*   チャートフィルタを実行した後に、フィルタリストのダイアログが自動的に閉じるようになりました。
*   ナンバーチャートにおいて、ダッシュボード配置時の視認性を高めるためにフォントサイズを指定できるようになりました。
*   ナンバーチャートにおいて、接頭辞や接尾辞の文字サイズや色を個別にカスタマイズできるようになりました。
*   バーチャートでロジカル型の割合を表示した際、詳細データにTRUEだけでなくFALSEの行も含まれて表示されるようになりました。
*   バーチャートで値が0の場合でも、チャート上に「0」の数値を表示できるようになりました。
*   ピボットテーブルにおいて、フォントサイズを小さく設定した際に行の高さが自動で縮小されるようになりました。
*   フィルタの「月の日」タイプにおいて、範囲指定ができる「範囲内」演算子がサポートされました。
*   散布図の軸に日付列を割り当てた際、設定から表示する日付の期間範囲を指定できるようになりました。
*   表示行数制限がかかっているピボットテーブルでも、エクスポート時には全行のデータを出力できるようになりました。
*   集計テーブルの「その他」グループ設定において、グループごとに頻度の高い値を残してそれ以外をまとめることができるようになりました。
*   集計テーブルやピボットテーブルにおいて、列を識別しやすくするための垂直方向の罫線を表示できるようになりました。

### アナリティクス

*   アナリティクスビューのメニューに、LightGBMが追加されました。
*   クラスタリングのプロパティから、現在はサポートされていない平行座標プロットに関する設定セクションが削除されました。
*   因子分析の結果において、因子得点をどのように解釈すべきかの説明ガイドが追加されました。

### ダッシュボード

*   ダッシュボード全体の背景色やチャートのボーダーの色を、設定ダイアログから自由に変更できるようになりました。
*   日本語UIを使用している場合、ダッシュボードのPDF印刷ダイアログも日本語で表示されるようになりました。

### ノート

*   ノートの目次（TOC）のスタイルが改善され、より使いやすいデザインが導入されました。
*   編集モードのチャートにおいて、クリック操作で「行の選択」と「チャート要素の選択」を簡単に切り替えられるようになりました。

### データソース

*   CSVインポート時に、読み込む最大行数を指定してインポート時間を短縮できるようになりました。
*   GitHub Issuesデータソースにおいて、日付範囲やステータスなどのパラメータを指定してデータを取得できるようになりました。
*   GitHubのデータソースにおいて、1万件を超える古いイシューを取得するためのフィルタパラメータがサポートされました。
*   データコネクションの選択肢から、古くなったMailChimpの項目が削除されました。
*   データベース接続ジョブにおいて、Rserveの状態が不安定になりSQLクエリがnullを返した場合に自動再起動するようになりました。

### テーブル・ビュー

*   テーブルビューのホバーポップアップでコピーボタンをクリックした際、「コピー完了」を示す視覚的なフィードバックが表示されるようになりました。

### パラメーター

*   単一選択パラメータのダイアログにおいて、ラベルの文言変更や入力フィールドの配置調整が行われ、使いやすくなりました。

### プロジェクト

*   JS APIで生成されたRスクリプトに対して、テストフレームワーク内でリンターによる検証ができるようになりました。
*   Mac環境で書き込み権限のないフォルダにプロジェクトを作成しようとした際、適切なエラーチェックが行われるようになりました。

### その他

*   Intel Mac環境での自動テスト環境構築を自動化するスクリプトとガイドが整備されました。
*   M1 Mac環境の自動テストにおいて、CSV保存時のエンコーディングや改行コードに関するテストの失敗が修正されました。
*   Rserveがクラッシュして接続エラーが発生した際に、自動でRserveを再起動できるようになりました。
*   アプリケーション終了時に、リソースを適切に解放するためのクリーンアップ処理が強化されました。
*   サマリービューのヘルプリンクの日本語化や、フィルタ・基準線ボタンのスタイル改善、GitHub連携のヘルプ更新などが行われました。
*   肥大化した古いログファイルを自動的に削除し、ストレージ容量を節約する仕組みが導入されました。

## 問題の修正

### AI プロンプト

*   AIプロンプトで時間型（hh\:mm\:ss）への型変換を指示した際、誤った関数が使われてエラーになる問題が修正されました。
*   AIプロンプトのエディタで複数行のコードを入力すると、テキストが領域外に溢れてカーソル位置が見えなくなる問題が修正されました。
*   AIプロンプトの実行時などに、存在しない「null」関数を呼び出す不正なスクリプトが生成されエラーが発生する問題が修正されました。

### AI 関数

*   AI関数のリクエスト実行中に、ネットワークエラーやサーバーからのデータ受信失敗により処理が中断される問題が修正されました。

### データソース

*   Excelインポートダイアログで最初のシートが空の場合、テーブルビューの横スクロールができなくなる問題が修正されました。
*   GitHubのデータインポートで全列の選択を解除した後に数列だけ選び直しても、不要な列までインポートされる問題が修正されました。
*   Googleスプレッドシートのインポート時に、ダイアログで指定したデータ型の変換が正しく反映されない問題が修正されました。
*   JSONインポート時に「データタイプを自動認識」のチェックを外していても、自動的に型変換が行われる問題が修正されました。
*   Rスクリプトのインポートダイアログにおいて、一度表示したプレビューデータが保持されない問題が修正されました。
*   インポートダイアログで「yyyy/m/d」形式の文字列を日付型に変換しようとすると、欠損値になる問題が修正されました。
*   クラウドストレージからCSVをインポートする際、プレビューの列制限を高く設定しても100列までしか表示されない問題が修正されました。
*   ファイルのインポート・マージ機能において、検索条件に濁点や半濁点を含む文字列を指定すると正しくファイルを抽出できない問題が修正されました。
*   リモートCSVのデータソースを変更した際、ファイルパスやプレビュー内容が新しいファイルの情報に更新されない問題が修正されました。


### データ加工

*   AIデータ加工の指示に全角の括弧記号を含めると、生成されるRスクリプトで半角に変換されてしまい正しく処理されない問題が修正されました。
*   Shiftキーを押しながらステップのトークン部分をクリックして複数選択しようとしても、単一選択になる問題が修正されました。
*   外れ値の除外機能が正しく動作しない問題が修正されました。

### チャート

*   「値を表示」のポジションを「自動」にした際、欠損値（NA）を持つデータのラベルが不適切な場所に表示される問題が修正されました。
*   1つの指標のみを持つピボットテーブルで総計を表示しようとすると、Rのエラーが発生して描画に失敗する問題が修正されました。
*   X軸に「表示する値の制限」を適用すると、色に割り当てた列のFactorレベルが消えてしまい、色の割り当てが変わる問題が修正されました。
*   Y軸マーカーのサブメニューとカラム選択リストの表示位置が、メインメニューの上端と合うように修正されました。
*   エリアチャートで繰り返し（リピート）を使用しX軸の同期を外した際、X軸の日付スケールの開始位置がずれて表示される問題が修正されました。
*   チャートのフィルタや基準線の設定を無効化していても、プレビュー画面の条件一覧に表示され続ける問題が修正されました。
*   テーブルチャートの保存処理に時間がかかり、別のタブへの切り替えに待たされる問題が修正されました。
*   ノートに挿入した地図チャートをイメージとしてエクスポートしようとすると、データが空であるというエラーで失敗する問題が修正されました。
*   バーチャートのX軸に基準線を設定した際、Y軸に複数の値を選択しているとラベルが重複して表示される問題が修正されました。
*   バーチャートやヒストグラムなどで、信頼区間と重なってしまう不要な矢印を非表示にできるようになりました。
*   ピボットテーブルで列の並び順を降順に変更した際、合計行の値がソート後の列に対応せず、不一致が生じる問題が修正されました。
*   ピボットテーブルの描画時に、指標の定義が不正な場合に数値判定処理でエラーが発生する問題が修正されました。
*   ピボットテーブルをExcelやクリップボードへエクスポートした際、合計や小計が含まれていない問題が修正されました。
*   円グラフを「繰り返す」で分割した際に、各グラフのタイトルが表示されない問題が修正されました。
*   散布図でY軸ラベルの小数の桁数を0に設定すると、詳細データを表示する際のフィルタ条件から小数点が落ちてデータが表示されない問題が修正されました。
*   散布図のY軸で小数の桁数を設定していると、データポイントをクリックしても詳細データが表示されない問題が修正されました。
*   集計テーブルをCSVエクスポートした際、グループ化された列の値が2行目以降欠損値になる問題が修正されました。

### アナリティクス

*   相関分析のアナリティクスビューにおいて、「テーブルを表示」をクリックしても読み込みが終わらずテーブルが表示されない問題が修正されました。

### ダッシュボード

*   ブラウザからダッシュボードをPDFとして出力する際に、チャートが正しくエクスポートされない問題が修正されました。
*   特定のRコードを使用したサンキーチャートで、プレビューやパブリッシュ時に中身が空になる問題が修正されました。

### ノート

*   2列レイアウト内に配置した数値チャートが、プレビューやパブリッシュ後の画面で下部が欠けてスクロールバーが表示される問題が修正されました。
*   Rスクリプトブロックを実行した際、エラーが発生しているにもかかわらず何も表示されない問題が修正されました。
*   ノートの2列レイアウトで特定の操作を行うと、右側のチャート領域が消えたりノートが白紙になったりする問題が修正されました。
*   ノートのRコードブロックで特定のパッケージを使用して図を描画しようとすると、プロセスが競合してエラーが発生する問題が修正されました。
*   ノートを開いた際に、データが存在するはずのチャートが「表示するデータがありません」と誤って表示される問題が修正されました。
*   ノート内のRコードブロックのプレビュータブにおいて、表示されているテキストを選択できない問題が修正されました。
*   ノート内の検索ボックスで文字を入力すると、すぐにフォーカスが外れて検索ができなくなる問題が修正されました。
*   指標付きの数値チャートを含むノートをWordへエクスポートした際、メインの値が誤った内容で表示される問題が修正されました。
*   数値チャートを含むノートをWordファイルにエクスポートしようとすると、型変換のエラーで失敗する問題が修正されました。
*   特定のノートを開こうとすると、エディター画面が真っ白になり操作不能になる問題が修正されました。
*   特定のマークダウンテキストをノートエディタに貼り付けた際、書式が正しく反映されず構文が無視される問題が修正されました。
*   絶対パスで画像を参照しているマークダウンをノートに貼り付けた際、画像が表示されない問題が修正されました。

### テーブル・ビュー

*   Windows環境のテーブルビューで一番下までスクロールした後にマウスホイールを操作すると、表示が急に一番上に戻る問題が修正されました。
*   テーブルのスクロールバーを最下部までドラッグした後にマウスホイールで上へスクロールすると、位置がリセットされる問題が修正されました。
*   テーブルのポップアップにおいて、テキストが長い場合に表示が切れたり枠外にはみ出したりする問題が修正されました。

### パブリッシュ

*   BigQuery OAuth接続を含むプロジェクトを別PCに移行してパブリッシュしようとすると、コネクションエラーが発生する問題が修正されました。
*   PDFエクスポート時に、数値チャートのサブ指標の書式崩れやピボットテーブルの色設定の欠落、地図の非表示が発生する問題が修正されました。
*   SQLインポートやパブリッシュのジョブをキャンセルする際、ステータス確認を待たずに一律1分間待機する仕様が改善されました。
*   ダッシュボードのパブリッシュなどの処理をキャンセルした際、完了までに非常に長い時間がかかる問題が修正されました。
*   ピボットテーブルをExcelエクスポートした際、特定の文字や多言語データが原因でファイル破損エラーが発生する問題が修正されました。

### パラメーター

*   セッションパラメータに依存する動的なパラメータにおいて、実行や再起動をしないと適切な値が代入されない問題が修正されました。
*   パラメータで今日以降の日付を指定して実行すると、処理が終わらない問題が修正されました。
*   日付を参照するパラメータにおいて、デフォルト値の更新やリセットが正しく反映されない問題が修正されました。

### プロジェクト

*   データフレームのメニューを開いた際、データフレーム名にマウスオーバーするとポップアップが重なってメニュー操作を妨げる問題が修正されました。
*   バージョンアップ後にプロジェクトを開くと、カスタムパッケージが削除されていないのに警告が表示される問題が修正されました。
*   履歴から状態を復元しようとした際、Gitのロックファイルが残っているとエラーで失敗する問題が修正されました。

### その他

*   Intercomのチャットダイアログにおいて、セキュリティ設定で許可されているにもかかわらずログファイルを添付できない問題が修正されました。
*   Windows環境の自動テストにおいて、ファイルパスの扱いに起因するテストの失敗が修正されました。
*   オフラインモードに設定してアプリケーションを終了しても、再起動時に設定が保持されずオンラインモードで起動する問題が修正されました。
...
* AI: AI関数: AI関数 の結果と実際の行数が一致しない場合の安定性を向上しました。
* AI: AI関数: 列名に日本語や数字が含まれている場合の処理を改善しました。


* アナリティクス: ランダムフォレストおよび XGBoost において、実行エラーを防ぐために無限大（inf）の値を自動的に除外するようになりました。
...
* データラングリング: ロングから: ロングからワイド を使ったワンホットエンコーディングをサポートしました。


* チャート: 実際のデータ範囲に関わらず、繰り返し年チャートで X 軸の範囲（例: 1/1〜12/31）を一定に保つオプションをサポートしました。
...
* ダッシュボード: 複数ページのダッシュボードを PDF にエクスポートできるようになりました。


* ノート: ノートの PDF エクスポートをサポートしました。


### 修正された問題

* データラングリング: ロングからワイド: 値に空文字列が含まれている場合、ワイド型への変換時にエラーが発生する問題を修正しました。
* データラングリング: TF-IDF: TF-IDF 計算時に document_id が正しくならない問題を修正しました。


* チャート: 表計算: 「欠損値を除外」が有効な場合に、他の列の値まで削除されてしまう問題を修正しました。
* チャート: バーチャート: 繰り返しと累積パーセンテージを使用した場合に、Y2 軸が 100% を超えてしまう問題を修正しました。
* チャート: ピボットテーブル: v14 以降で発生していたパフォーマンス低下を修正しました。
...
* ダッシュボード: URL で指定したパラメーター値がダッシュボード結果に反映されない問題を修正しました。
* ダッシュボード: Exploratory サーバー上ですでにパラメーターを実行したことがある場合に、URL で指定したパラメーター値が反映されない問題を修正しました。


* ノート: ノートを PDF にエクスポートした際に、チャートや画像がページ区切りで分割されてしまう問題を修正しました。
...
* チャート: ピボット / 集計テーブル: テーブルを画像ファイルとしてエクスポートできるようになりました。


* ノート: チャートおよびアナリティクスで、正方形サイズを設定できるようになりました。

### 修正された問題

* 一般: Windowsアップデート後に、ExploratoryがWindows環境で起動しなくなる問題を修正しました。


* チャート: 散布図: データが部分的に重なっている場合に、2番目のY軸の列に対するトレンドラインの位置が正しく表示されない問題を修正しました。
...
* チャート: リファレンスライン: リファレンスラインのボックスをドラッグ＆ドロップで並び替えられるようになりました。
* チャート: リファレンスライン: mean 以外の Roll 関数をサポートしました。


* ダッシュボード: テキストパネル: パラメーターを使って、URL ベースで画像を動的に表示できるようになりました。
* ダッシュボード: ダッシュボード上のチャートタイトルで太字フォントをサポートしました。

### 修正された問題

* データソース: Google Cloud Storage: バケット内に多数のファイルがある場合に、UI 上で http_400 エラーが発生して一覧表示できず、ファイルをインポートできない問題を修正しました。


* チャート: 特定の条件下で、チャートの並び順が変わってしまう問題を修正しました。
* チャート: 折れ線: ホバーモードで同じ X 軸上にすべての値を表示している際、欠損値があると次の値が正しく表示されない問題を修正しました。


* プロジェクト: コラボレーション: プロジェクトを閉じずにアプリを終了した場合、編集中を示す緑色のランプが点灯したままになる問題を修正しました。
* プロジェクト: Exploratory 14.1 へアップグレード後に、"invalid multibyte character in parser (C:\Program Files\Exploratory\lib\library.r:4591:61" というエラーが発生して、プロジェクトを開けない問題を修正しました。
