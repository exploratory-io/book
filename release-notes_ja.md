...
* データソース: Oracle: システム設定のデータベースタブから、Oracleデータベース用のエンコーディング (NLS_LANG)を設定できるようになりました。

### 修正された問題

* データラングリング: 日付型/日付時間型の列から「曜日」を「抽出」したときに、新しい列名が「元の列名_wday」ではなく、「calculation_1」として表示される。
 ...
 * 一般: プロジェクトの新規作成時にエラーが発生することがありました。
