...
- characterなど、カテゴリカルな列がColor軸に指定されたときに、複数の色パターンから色を選ぶことができるようになりました。また、十六進コードを使って、色のカスタマイズができるようになりました。
- Dataインポートダイアログで、戻り値のデータの行数が表示されるようになりました。

### Bug fixes

- Windowsで大容量のデータをインポートできるようになりました。修正される前は、Windows環境だけ2 GB以上のデータをインポートしようとするとエラーになりました。
- 入力画面で、ブランチ名を閉じたことのあるブランチ名に対してエラーバリデーションをかけました。今も、エラーを表示しますが、同じダイアログで名前をアップデートできるようになりました。
