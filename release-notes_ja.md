...
* アナリティクス: トピックモデルと同様に、「単語のカウント」アナリティクスでも単語化済みのデータを使用できるようになりました。
...
* データ・ラングリング: Custom Step: custom R script ステップが library(dplyr) 呼び出しから始まると、Error in library(., dplyr) : there is no package called ‘.’ エラーで失敗する問題を修正しました。
