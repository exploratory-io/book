...
今回のリリースの最大の特徴は、プロジェクトレベルのコラボレーションに対応したことです。Exploratoryサーバーにプロジェクトをパブリッシュして、他の人にプロジェクトを共有できるようになりました。

### 機能強化

* コラボレーション: プロジェクトをパブリッシュして、他の人と共有できるようになりました。
* コラボレーション: 他の人が更新してパブリッシュしたプロジェクトを使って、自身のプロジェクトを最新の状態に更新できるようになりました。
* コラボレーション: パブリッシュされたプロジェクトはバージョンごとに管理されているので、過去の特定のバージョンに復元できます。
* コラボレーション: チームとしてプロジェクトをパブリッシュできます。これにより、チームメンバーとのコラボレーションが容易になります。


* データソース: Amazon Aurora: SSL CAファイルのパラメーターをサポートしました。
* データソース: Google Analytics: 「データのサンプル」をするかどうかを選択できるようになりました。
* データソース: Google Sheets: サーバーでシート名を検索できるようになりました。
* データソース: Google BigQuery: ページサイズに関するエラーメッセージを改善しました。


* サマリビュー: 表示する列/行において、最初または最後からN列/行を選択できるようになりました。
* サマリビュー: 相関モード: アルファベット順にソートできるようになりました。
* サマリビュー: 相関モード: 選択した指標に基づいてすべての列をソートし、最初/最後のN列のみを表示できるようになりました。


* テーブルビュー: 表示する列の数を選択できるようになりました。


* データラングリング: 列の並べ替えのメニューに「先頭」および「最後」のオプションを追加しました。


* ダッシュボード: 「指定した高さに収める」を選択して、行の高さをドラッグ＆ドロップで調整する際に、他の行の高さも維持するようになりました。


* チャート: 箱ヒゲ図で、値に'inf'を含む場合、それらの値を自動で除外するようになりました。
* チャート: 繰り返しを選択したときに、ツールバーに 「X軸を同期」オプションが表示されるようになりました。


* パラメーター: ステップデータを自動的にキャッシュすることにより、インタラクティブモードを有効にする際のパフォーマンスを向上させました。
* パラメーター: 無効なパラメーターを検出し、より分かりやすいエラーメッセージを表示するようになりました。


* プロジェクト: プロジェクトの作成日、ファイルサイズ、現在のメモリ消費量など、より多くの情報を確認できるようになりました。


* インストール: より直感的にインストールをできるようにインストール時のUIを再設計しました。
...
* データラングリング: 列名を変更したときに発生するエラーの処理を改善しました。
* データラングリング: マージ: デフォルトの設定で 「データ・タイプを強制的にそろえる」のチェックを外すことで、より高いパフォーマンスを得られるようになりました。
...
* チャート: 「一意な値のチェック」ダイアログで、「これ以降、この列では一意な値のチェックをスキップする」にチェックをつけても、他の列の割り当てを更新すると、「一意な値のチェック」ダイアログが表示される。
