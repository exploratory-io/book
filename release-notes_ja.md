...
* AI: AIプロンプト: 入力フィールドと実行ボタンの表示位置がずれていました。

* データラングリング: ウィンドウ計算: グループにCharacter列が設定されている場合、POSIXct列を持つ値フィールドが未選択と表示されていました。

* チャート: マーカーショートカットメニューの導入後、マーカースタイルメニューにアクセスできなくなっていました。
* チャート: チャート画像のエクスポートに失敗した場合、新しいファイルとして保存を試みるようになりました。
* チャート: ピン留めされたチャートが無効な前ステップに依存している場合、ステップキャッシュの保存が失敗していました。
* チャート: ピボット: NA値を含むピボットテーブルデータをCSVにエクスポートする際、型の不一致エラーで失敗していました。
* チャート: 設定していないX軸参照線がチャートに表示される問題がありました。
* チャート: チャートの画像エクスポートが失敗した場合、ファイルとフォルダのパーミッションを確認し、再試行し、それでも失敗した場合は適切なエラーを表示するようになりました。

* アナリティクス: "train_model: multiclass training requires more than one class" エラーが発生していました。

* ノート: TOCが長すぎる場合、残りのTOCをスクロールして表示できませんでした。
* ノート: AIノートエディター: 下線が切れていました。

* パラメーター: Between演算子でパラメーターを選択した際、1つのみ表示されるべきところにLOVが2つ表示されていました。

* 一般: パブリックプランでExploratoryを開こうとすると、「Cannot read properties of undefined」エラーがログに記録されていました。
