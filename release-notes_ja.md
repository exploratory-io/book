...
*   生成されたRスクリプトの明確な解説を提供する「コードを説明」ボタンを追加しました。ユーザーは自身の操作によって生成されたRコードを容易に理解できるようになります。

### データソース

*   ダッシュボードのパブリッシュ時に無効な接続参照を自動的に検知・修正するよう、OAuth接続の処理を強化しました。

### 共通機能

*   列選択ダイアログにおいて、ドラッグ＆ドロップで列選択エリアのサイズを変更できる機能を追加しました。

### データ加工

*   ステップダイアグラムにおいて、親データフレームとブランチデータフレームの間に異なる色を設定しました。

### チャート

*   複数回答の列を使用するピボットテーブル向けに「行数設定」ダイアログを追加し、元の行数の表示ができるようになりました。展開後の件数ではなく展開前の元の行数を表示することを選択できるようになり、アンケート分析で正確な回答者数をレポートすることが容易になりました。
*   ピボットテーブル、集計テーブル、テーブルの拡大表示内において、ドラッグによる列幅の変更を可能にしました。

### アナリティクス

*   相関：ヒートマップチャートにおける変数のグループ化ロジックを改善しました。相関係数の近接度に基づいて変数がグループ化され、並べ替えられます。
*   潜在クラス分析（LCA）：潜在クラス分析（LCA）において、数値型の変数を選択できるようになりました。自動的にファクター型（順序カテゴリカルデータ）に変換されます。
*   決定木：決定木のツールチップから条件テキストを直接コピーできるようになりました。
*   K-Medoids：K-Medoidsのクラスタープロファイルのレーダーチャートにおいて、「すべての変数を表示」のデフォルト設定をTRUEに更新し、クラスタリングに使われたすべての変数がデフォルトで表示されるようにしました。
*   アナリティクスレポート本文内のP値（統計的有意性を測る指標）およびパーセンテージの書式を標準化し、0.001未満の値を「< 0.001」および「< 0.1%」と表示するようにしました。

### ノート

*   ノートをPDFにエクスポートする際、H1見出しの前の自動改ページを無効にするオプションを追加しました。これにより、不要な空白やタイトルのみのページが発生するのを防ぐことができます。

## 不具合の修正

### サマリビュー

*   サマリビューにおいて、0と1のみを含む数値列に対して「詳細を表示 / 非表示」メニューが機能せず、アサーションエラーが発生する問題を修正しました。

### データ加工

*   行フィルタダイアログにおいて、複製アイコンが誤った位置に配置されていた問題を修正しました。
*   「値の順序を設定」ダイアログ経由で新しいステップを追加した際、新しく作成された列が選択した列のすぐ右側ではなく、最後の列として配置されていた問題を修正しました。
*   ステップダイアグラムにおいて、新しく作成されたブランチが即座に表示されない、コントローラーがダイアログのフッターと重なる、ノードを接続する線が短すぎて矢印が適切に表示されないといった問題を修正しました。
*   ステップダイアグラムにおいて、無効化されたステップの表示・非表示を切り替えると、ステップノードの位置がずれ、意図した位置から大きく離れて表示される問題を修正しました。

### チャート

*   ピボットテーブルにおいて、欠損値（NA）を含むセルで「詳細を表示」をクリックした際、生成されたフィルター条件がNA対応の演算子を使用せずリテラル文字列と比較していたため、表示される行が0行になっていた問題を修正しました。
*   Exploratoryのアップグレードに伴いデフォルトのカラーパレットが更新されたことで、既存のピボットテーブルのカラーパレットが意図せず青から緑に変更されてしまう問題を修正しました。以前のバージョン（v15.x以前）で作成されたピボットテーブルには、新しいデフォルトカラーパレットが適用されないように更新しました。

### アナリティクス

*   順序ロジスティック回帰において、チャートの向きを切り替えた際にチャートの色が予期せず変更されてしまう問題、およびデフォルトでチャートが垂直方向になっていなかった問題を修正しました。
*   UI上でz値統計量のラベルが適切に翻訳されていなかった問題を修正しました。

### パラメータ

*   「equal\_or\_all」のフィルター演算子がパラメーターで使用された際、空またはNULL値を選択すると、すべての値として扱うか適切に処理する代わりに、サイズ0のベクトルエラーでアプリケーションがクラッシュする問題を修正しました。
...
*   AIデータダイアログ内のRスクリプトエディタにおいて、`read_csv()` などの一般的なR関数が正しくハイライトされない問題を修正しました。
*   Snowflakeのパッケージ命名規則およびドライバ内部の変更により、Snowflake ODBCドライバの自動セットアップが失敗する問題を修正しました。

### データソース

*   Google BigQueryにおいて、セッションパラメータを使用したクエリが、プレビューでは正しく表示されるものの、インポート後にデータが返されない問題を修正しました。

### データラングリング（データ加工）

*   日付フィルタの「月 - 名前 (1-12)」が、Windowsでは動作するもののmacOSで機能しない問題を修正しました。
*   キャッシュが古い状態（黄色）のステップを再実行した際、「キャッシュ日時」のタイムスタンプが更新されない問題を修正しました。
*   「値を置換」ダイアログにおいて、対象の列がFactor（順序付きカテゴリ）型であっても、他のFactor列を選択肢として表示できない問題を修正しました。
*   日本語環境において、フィルタダイアログの検索ドロップダウンが、IMEの漢字変換を確定した際に意図せず閉じてしまう問題を修正しました。
*   ステップダイアグラムにおいて、特定のノードのヘッダー部分にマウスを置いた際、ツールチップが表示されない問題を修正しました。

### チャート

*   「繰り返し」レイアウトにおいて、1行あたりのチャート数を1または2に設定し、かつグループ数が多い場合に、チャートが大きく重なってしまう問題を修正しました。
*   散布図において、データポイントをクリックして「詳細を表示」ダイアログを開いた後、色の不透明度設定が無視される問題を修正しました。
*   サムネイルキャプチャ処理の影響により、チャートタブの切り替え動作が遅くなっていた問題を修正しました。
*   ピボットテーブルにおいて、ウィンドウ計算と「値を制限」を組み合わせて使用した際、列の合計値が正しく更新されない問題を修正しました。
*   ピボットテーブルにおいて、複数の変数を選択した際、「変数の値」の表示順序がFactor（順序付きカテゴリ）のレベル順を無視してしまう問題を修正しました。
*   ナンバーチャートのサムネイル画像がインターフェース上で大きく表示されすぎる問題、およびピボットテーブルとテーブルのサムネイルに枠線が表示されない問題を修正しました。

### アナリティクス

*   アナリティクスビューを開く際、または既存のアナリティクス項目を削除する際に、大幅なパフォーマンスの遅延が発生する問題を修正しました。
*   アナリティクス・タイプのドロップダウンにおいて、「検出力分析」と「A/Bテスト」のサブメニューが画面下部で切れてしまう問題を修正しました。
*   決定木：CART決定木において、最初の分割以降、"TRUE"（真）のノードが常に左側に配置されない問題を修正しました。
*   決定木：CHAIDのツリー図において、年齢グループなどのカテゴリ順序が正しくない問題、およびツールチップに値が表示されない問題を修正しました。
*   CHAID決定木レポートにおいて、列名に含まれるカンマがピリオドに誤変換され、テーブル内で正しく表示されない問題を修正しました。
*   K-Means クラスタリング：重複行が多いデータセットにおいて、候補となるクラスター数がユニークな観測値数を超えてしまい、エルボー法がクラッシュする問題を修正しました。
*   K-Means クラスタリングにおいて、特定のデータセットでレーダーチャートが正しく描画されない問題を修正しました。
*   K-Means クラスタリングにおいて、「最大反復回数」の設定が無視され、ユーザーの入力に関わらず常にデフォルトの回数で実行される問題を修正しました。
*   K-Medoids クラスタリングのレーダーチャートにおいて、変数が類似度順にソートされておらず、K-Meansと比較して解釈が困難であった問題を修正しました。
*   相関分析：分析条件と出力データの不一致により、サマリーに誤った情報が表示される問題を修正しました。
*   主成分分析（PCA）： 「正規化」オプションがオフの際、固有値テーブルのカイザー基準の列が空白になる問題を修正しました。
*   主成分分析（PCA）：ピアソン相関を用いてカテゴリ変数を分析した際、符号付き負荷量（Signed Loadings）チャートがエラーになる問題を修正しました。
*   因子分析：回転手法に「Geomin (斜交)」を選択すると、内部パラメータ名の誤りにより分析がクラッシュする問題を修正しました。
*   対応分析：1次元のみのモデルにおいて、座標テーブルのラベルが標準の「次元 1」ではなく、内部的なRの変数名で表示される問題を修正しました。
*   ロジスティック回帰および順序ロジスティック回帰：変数が1つしかない場合に多重共線性の情報を表示しようとしてエラーになる問題を修正しました。
*   2群間の統計検定：グループ化されていない分析において、フィルタリングによって一方のグループのデータがすべて削除された場合に、不明瞭なエラーでクラッシュする問題を修正しました。
*   仮説検定：実行時に列の型チェックが行われず、上流のステップでデータ型が変更された場合に、非数値データで無効な結果が生成される問題を修正しました。
*   t検定、ANOVA、カイ二乗検定の確率分布チャートにおいて、統計値が正しくなく、ランダムな垂直線が表示される問題を修正しました。
*   集計済みデータを用いたt検定において、設定された「帰無仮説の平均の差」が計算時に無視され、差をゼロとした統計量とp値が算出される問題を修正しました。
*   集計済みデータを用いた2標本t検定において、両方のグループの分散がゼロの場合に、内部エラーや無効な統計量が発生する問題を修正しました。
*   ANCOVA（共分散分析）：Rのプリプロセッサにおける列名の不一致により、共変量が選択されていない場合に「平均値（調整済み）」チャートが空になる問題を修正しました。
*   ANCOVA（共分散分析）：共変量に欠損値（NA）が含まれる場合、調整前と調整後の統計量で行数が一致しない問題を修正しました。
*   ANCOVA（共分散分析）の「残差 vs 予測値」チャートにおいて、残差の線が正しく表示されない問題を修正しました。
*   ANOVA（分散分析）：共変量が定数であるか多重共線性があり、かつ交互作用がオフの場合に、分かりやすいメッセージではなく生のRエラーが返される問題を修正しました。
*   ANOVA（分散分析）：カテゴリ列が論理型、日付型、またはPOSIXct型の場合に、不明瞭なエラーでモデル作成が失敗する問題を修正しました。
*   One-Way ANOVA（一元配置分散分析）：他のグループに十分なデータがあっても、少なくとも1つのグループのデータが1件しかない場合に、生のRエラーで失敗する問題を修正しました。
*   クラスカル・ウォリス検定：カテゴリ列がFactor（カテゴリ）型でない（数値や日付など）場合に、事後検定（Post-hoc test）で失敗する問題を修正しました。
*   正規性検定：選択した列のうち1つでもエラー（データ数が少なすぎるなど）が発生すると、すべての列の検定が中断される問題を修正しました。
*   正規性検定：選択した列が単一の値しか持たない（縮退している）場合に、結果が表示されない、またはクラッシュする問題を修正しました。
*   正規性検定：欠損値（NA）を除外する前にサンプリングが行われていたため、乱数のシード値によって結果が不安定になったり失敗したりする問題を修正しました。
*   正規性検定のUIにおいて、最小要件であるサンプルサイズ3未満の設定が可能になっており、実行時エラーが発生する問題を修正しました。
*   1標本の比率検定において、基準となる確率が0または1の場合、内部のR関数が数値ではなく論理値のp値を返すためエラーになる問題を修正しました。
*   1標本t検定において、無限大（Inf）の値を除外せずに要約統計量に含めてしまい、無効な結果が算出される問題を修正しました。

### パラメータ

*   ダッシュボードのパラメータ値リストが、パブリッシュ時やサーバーでの初回アクセス時に古い状態に戻ってしまう問題を修正しました。

### プロジェクト

*   特定のプロジェクトを閉じる際、処理の非効率性により過度な時間（例：25秒など）がかかる問題を修正しました。
...
*   他のユーザーから共有されたインポート済みコンテンツの接続先を選択する際、接続ダイアログにホスト名とデータベース名が表示されるようになりました。これにより、既存の接続から正しいデータソースを正確に特定・選択できるようになり、共有された作業をインポートする際のミスを防ぐことができます。

### データラングリング（データ加工）

*   ステップダイアグラム内で、非表示（無効化）に設定されたステップを隠すオプションを追加しました。これにより、アクティブなデータパイプラインをよりすっきりと表示させることができ、第三者が実際のデータ処理ロジックを理解しやすくなります。

### チャート

*   「繰り返し」セクションで「複数の変数」を使用する際、表示名を編集できるオプションを追加しました。これにより、チャート上のラベルを直接カスタマイズできるようになります。
*   ピボットテーブル：列を一時的に削除しても、割合（比率）の計算方向の設定が保持されるように更新しました。これにより、設定を失うことなく異なる表示モードを切り替えることができ、繰り返しの手動調整を大幅に削減できます。
*   ピボットテーブル：ターゲット合計の設定から、不要な「総合計」オプションを削除しました。混乱を招きやすく、あまり有用ではなかったオプションを取り除くことで、UIを簡素化しました。
*   ピボットテーブル：フォーマットで「割合（パーセンテージ）」を選択した際、デフォルトで常に小数点以下1桁まで表示するように更新しました（例：「6.12%」ではなく「6.1%」）。これにより、レポート内でのデータ表示に一貫性が保たれ、値を正確に比較しやすくなります。

### アナリティクス

*   K-Medoidsクラスタリング：行数の多いデータに対するK-Medoidsクラスタリングアルゴリズムの実行パフォーマンスを改善しました。数千行以上のデータでもより高速に処理できるようになり、大規模なアンケート回答データなどの探索的分析をより効率的に行えます。

### ノート

*   ノートエディタでチャートを挿入する際、タイトルと一緒にチャートのコメントも自動的に含まれるように更新しました。これにより、チャート追加のさいに重要な付属情報が保持され、説明文をノートに手動でコピーする手間を省くことができます。

## 不具合の修正

### データソース

*   「スキーマ情報の取得」ボタンにおいて、「更新」ボタンを先にクリックしない限り、更新された接続詳細が反映されない問題を修正しました。

### データラングリング（データ加工）

*   AIサマリーおよびノートのエクスポートワークフローを通じて生成されたPDFドキュメントにおいて、ステップダイアグラムが欠落していた問題を修正しました。
*   別のデータフレームに属するステップをクリックするたびに、ステップダイアグラムのズームレベルが予期せずリセットされる問題を修正しました。
*   ステップにコメントを追加した際、ステップダイアグラム上で実際の操作サマリーが隠れ、コメントテキストに完全に置き換わってしまう問題を修正しました。

### チャート

*   チャートのX軸ラベルにおいて、長いラベルを持つバーが多い場合に、ラベルが重なったり、折り返しや角度の調整が正しく行われない問題を修正しました。
*   アナリティクスやチャートがたくさん含まれるデータフレームをコピーしようとすると、「RangeError: Invalid string length」というエラーで失敗する問題を修正しました。
*   ピボットテーブル：単語が特殊文字で連結されている場合に、テーブルの行ヘッダーテキストでテキストの折り返し機能が動作しない問題を修正しました。
*   ピボットテーブル用に生成されたサムネイル画像が、歪んだり正しく表示されない問題を修正しました。

### アナリティクス

*   順序ロジスティック回帰：数値型の目的変数が、1〜5の尺度のような小さな範囲の値であっても、割り当て時に誤ってビン分割（グループ化）されてしまう問題を修正しました。

### ノート

*   ノート内のテキストの一部を選択した状態でAIノートエディタを起動しようとすると、エディタが開かない問題を修正しました。

### インストール

*   Intel Macにおいて、prophetパッケージが正しくインストールできなかったため、v16へのアップグレード中にRパッケージのインストールが失敗する問題を修正しました。
...
*   `count_if`やその他の`count_x`関数を簡素化し、引数として`row_number()`関数を必要としないようにしました。これらは、チャート内のカスタム計算や、「集計（Summarize）」、「計算を作成（Create Calculation）」ステップで使用できる関数です。
...
*   ノートのプレビュー時に「このバージョンのExploratoryは、今後のmacOSリリースでは開けません」という警告メッセージが表示される問題がありました。調査の結果、Exploratory自体はRosettaを必要とせずApple Siliconに完全対応していましたが、ノートで使用される同梱のPandocバイナリが依然としてx86\_64（Intel用）実行ファイルであったことが判明しました。これをApple ARM互換バージョンに差し替えました。
...
* チャートのAIサマリーで、「予測範囲」ではなく「推定範囲」という表現を使用するように変更しました。また、使用している確率タイプ（例：90%範囲）をサマリーテキストに含めるようにしました。

## 修正された問題

### プロジェクト

* Exploratory Serverから更新されたプロジェクトをインポートしようとした際、プロジェクト一覧の「更新」ボタンをクリックすると「Project Not Found」エラーが発生する問題を修正しました。
...
* v15へアップグレード直後、または既存プロジェクトを開いた際に、「File already exists」エラーが発生し、アプリケーションにアクセスできなくなることがある問題を修正しました。

### データソース

* データフレームがたくさんあり、それらの関係が複雑なプロジェクトでSQLデータを再インポートする際、不要なカタログ読み込み処理によりUIが最大1分ほどフリーズすることがある問題を修正しました。読み込みロジックを最適化し、アプリケーションの応答性を維持したまま、遅延なく再計算が開始されるよう改善しました。
* v15へのアップグレード後、SQLデータの再インポート速度が、従来の約5秒から1分以上に低下していた問題を改善しました。
* 後続ステップのエラーによってデータ再インポートが失敗した際、実際とは異なる操作を参照した分かりづらいエラーダイアログが表示される問題を修正しました。

### チャート

* フィルターダイアログの「X」ボタンをクリックしても、ウィンドウが閉じないことがある問題を修正しました。
* チャートやアナリティクスレベルにおいて、フィルターやステップをドラッグ中に境界線が過剰に点滅する問題を改善しました。
* フィルターダイアログを入力せずに閉じた後、次回クリック時にダイアログが開かず、2回目のクリックが必要になる問題を修正しました。

### ノート

* ノートの「名前を編集」メニューをクリックした際、名前変更ダイアログとノートエディタが同時に開いてしまう問題を修正しました。
* v15へアップグレード直後、一時的にノートの内容が空に見えることがある問題を修正しました。（プロジェクトを再起動すると内容は再表示されていました。）
...
*   基盤となるRおよびRパッケージのアップグレードにより、特に「表計算」や`case_when`や `ifelse` といった関数の処理におけるパフォーマンスが向上しました。この改善は、「計算を作成」、「表計算」、「条件付き計算」、「条件付きで値を置換」などのステップに関係します。

*   AI プロンプト for データラングリング：AIが生成する説明をより簡潔にし、生成速度を向上させるため、「具体的な例」のセクションを削除しました。
...
*   ノートエディタに「フローダイアグラム」ウィジェットを追加しました。[Mermaid JS](https://mermaid.js.org/intro/syntax-reference.html)を使用しており、[Mermaid JSの構文](https://mermaid.js.org/intro/syntax-reference.html)に従ってテキストを入力することで、ノート内にフローダイアグラムを視覚化する図を直接生成できるようになります。
...
* チャート: ピボットテーブル: チャートのプロパティから、列のカテゴリ間に縦の線を表示できるようになりました。
...
* チャート: 散布図: X軸とY軸に同じカテゴリ型の列を割り当て、「色で分割」と「繰り返し」に列を選択すると、Y軸のテキストの順序がおかしくなる。
...
* チャート: バー: 欠損値（NA）を含む日付列を色に割り当てたとき、欠損値（NA）がカテゴリとして表示されませんでした。
...
* チャート: 10以上あるカテゴリにグラデーションのカラーパレットを割り当てたときに、「色、グループ、並び順」のダイアログ内のカラーパレットと、グラフのパレットが一致しない。
* チャート: その他グループ: カテゴリの数が多い場合、ダイアログで表示されるカテゴリのリストが見切れてしまう。
...
* データラングリング: カテゴリを作成: 「ラベルの表示方法」のパラメーターを追加し「不等号の記号付き」の表示をサポートしました。
...
* チャート: X軸に数値型の列を選択した際、一意な値の数が200以上だったときに数値をカテゴリに分けるようになりました。
...
* データラングリング：カテゴリを作成：範囲外の値のカテゴリのラベルに誤りがある。
* データラングリング：Windowsで、カスタムRコマンドで長いスクリプトを使用するとエラーコード127で失敗する。


* チャート：バーチャートで、色に数値列を割り当て、カテゴリ分けのタイプを変更し、色の列の選択を解除すると「Invalid argument was passed to returnsNumericValue」と表示される。
...
* データラングリング: カテゴリを作成: 「範囲の端の扱い」に「下端の値を含む」を選択したときに、カテゴリ名に誤った不等号が使われている。
* データラングリング: カテゴリを作成: カテゴリのタイプに「手動」を選択したときに、「ラベル」を設定すると、プレビューが空白になる。


* サマリー: 行フィルタを削除するときの確認ダイアログに不要なHTMLタグが表示される。


* チャート: 数値をカテゴリ化した時のバケットのラベルは、よりわかりやすい表現で表示する必要があります。例えば、「2000<=」ではなく、「2000以上」といった表記に変わります。
* チャート: 色: 数値をカテゴリ化した際に、「等幅」を使ってバケットを作成した際に、バケット数を5より多く設定し、カラーパレットに「レッド / ブルー」を選択すると、５色のパレットが繰り返される。
* チャート: 色: 数値をカテゴリ化した際に、「等幅」を使ってバケットを作成し、「0を中央にする」のオプションが適用されていると、指定されたデータ範囲を使用してバケットが作成され、パレットの中心の色が次のデータが次のデータが含まれるバケットと揃う必要がある。
...
* チャート: 色で数値列をカテゴリ化した場合、「値の表示」の「チャートと同じ色」のオプションが機能しない。
...
* データラングリング: カテゴリを作成: カテゴリを作成時のアウトプットのフォーマットが、チャートでのカテゴリ作成と一致するように変更されました。
...
* データラングリング: カテゴリを作成: 区切り値の小数の桁数を指定できるようになりました。
...
* チャート: バー:  「色で分割」に列を選択したときに、並び替えの設定から、「最初のカテゴリで並び替え」ができるようになりました。
...
* チャート: ハイライト: 複数のカテゴリに同じ色を割り当てた場合、凡例の順序が正しくありませんでした。
...
* チャート: 数値のカテゴリ化で等間隔の間隔に負の数値を入力するとエラーダイアログが表示される。
...
* チャート: ピボット: 集計テーブル: 小数点以下の桁数の設定が、「<0.63」などのカテゴリ化したラベルの数値に適用されない。
...
* チャート: X軸に数値列を選択しカテゴリを適用したときに、X軸のラベルの小数点以下の値が6桁以上の場合、小数点以下の6桁と7桁目の間に不要な「.」が表示される。
...
* データラングリング: カテゴリを作成: 範囲の端の扱いのデフォルトを「下端の値を含む」に変更しました。
...
* チャート: バー: 各カテゴリが1行しかないデータを使って、カテゴリ型の列を「X軸」と「色（グループ化）」に選択して、さらにY軸に数値型の列を選択し、集計関数を標準偏差に設定した状態で、「色（グループ）化」において表示する値の制限に「上位」を選択すると、「forcats::fct_reorder(): ! Arguments in ... must be used.」というエラーが表示される。
...
* データラングリング: 条件を設定して置換、カテゴリを作成、テキストデータの加工: 選択した列の直後に新しい列が作成されるようになりました。
...
* チャート: カテゴリのタイプの「等間隔」でラベルが設定できない。
...
* チャート: X軸を「等間隔」によってカテゴリ化している場合、X軸の小数点以下の桁数の設定が機能しない。
...
* データラングリング: カテゴリを作成: カテゴリの作成時に「範囲の端の扱い」およびその他のパラメーターをサポートしました。
* データラングリング: カテゴリを作成: 「ファクター型として順序を保持する」のチェックボックスをオフにしても、ファクター型の列が作成される。
...
* チャート: エリア:  X軸にNumeric（数値）型またはDate（日付）型の列を選択し、色（グループ化）に列に選択したときに、全てが欠損値になるカテゴリがあると、最大のカテゴリとして凡例に表示されてしまう。
...
* チャート: ピボット / 集計テーブル: カテゴリ値の色がfactor型またはテキストの順序を尊重していない。
...
* チャート: 「その他」グループはカテゴリの最後に表示するように変更。
* チャート: 数値のカテゴリ化の「等幅」で範囲の端の扱いに「上端を含める」を設定し、「範囲の下限」に値を設定しなかった場合、最小値が含まれない。
...
* チャート: 数値のカテゴリ化のオプションで「等幅」で値の「範囲」を設定できるようになりました。
* チャート: 数値のカテゴリ化のオプションで「等間隔」がサポートされました。
...
* データラングリング: ビン（カテゴリ）の作成: 整数型の数値列でビン（カテゴリ）を作成したときの値の範囲の表記を「(1,3]」から「1-3」に変更しました。
...
* アナリティクス: 生存曲線: 色で分割に数値型の列を選択して、スケールにカテゴリを設定したときのデフォルトのカラーパレットを、ブルーのグラデーションのパレットに変更しました。
...
* チャート: 順序つきカテゴリ（factor）型の列を色に割り当てた場合の、デフォルトのカラーパレットをグラデーションカラーに変更しました。
...
* チャート: 「カテゴリの作成」で「手動」の区切りタイプを選択した際、凡例の順番がおかしい。
