...
- ローカルのデータ・タイプの更新機能をサポートしました。今なら、ローカルファイルのデータを更新するために、Stepの上のRefreshと書かれたリンクをクリックするとデータを更新することができます。
- Windows版で、RとGitのインストールする場所のカスタム機能をサポートしました。
- コマンドビルダーで、テキストをトークン化して、感情値を得ることができるようになりました。今なら、列のヘッダーのドロップダウンメニューからテキストデータをトークン化するコマンドを作れて、トークン化されたテキストの感情値を得ることができます。
- ユーザネームとパスワードを入力しないでも、Mongodbのデータをインポートできるようになりました。

### Bug fixes

- app消したプロジェクトが、アプリケーションをリスタートするまで消えなかった問題を修正しました。
