...
* AI: AI プロンプト: Rコードブロックで「プレビュー」ボタンをクリックして、提案されたRスクリプトの出力をプレビューできるようになりました。さらに、R式を選択して部分的にプレビュー実行することもできるようになりました。
* AI: AI プロンプト: プレビューエリアで「自動修正」ボタンをクリックして、提案されたRスクリプトの問題を自動的に修正できるようになりました。
* AI: AI プロンプト: 入力内容に一致する過去のプロンプト履歴を表示できるようになりました。
* AI: AI プロンプト: データフレーム名の提案で、"$"記号に加えて"@"記号も使用できるようになりました。


* データラングリング: エクスポート: CSVエクスポート時に、エンコーディング（BOM付き）や改行コード（LF/CRLF）を選択できるオプションが強化されました。


* チャート: ピボットテーブル: 列プロパティダイアログから、列タイトルテキストの配置をコントロールできるようになりました。
* チャート: ピボットテーブル: チャートプロパティダイアログから、列カテゴリー間に縦グリッドラインを表示できるようになりました。


* ダッシュボード: 「別のチャートを選択」ボタンから、チャートを別のものに差し替えられるようになりました。



### 修正された問題

* AI: AI プロンプト: AIからのストリーミング結果を受信中に自動で最下部までスクロールするようにサポートしました。
* AI: AI プロンプト: Exploratory Serverでメールアドレスを変更した後、Exploratory Desktop上でAI プロンプトが動作しない問題を修正しました。
* AI: AI プロンプト: パッケージがデフォルトでロードされていない場合でも、関数名にパッケージ名を自動的にプレフィックスするようになりました。
* AI: AI プロンプト: サーバーが応答しない場合（例: 529 Overloaded Error）に対するエラーハンドリングを改善しました。
* AI: AI プロンプト: 線形回帰モデルの作成時に画面が空白になる問題を修正しました。
* AI: AI プロンプト: データフレームをマージしようとした際に不要なスクリプトが生成される問題を修正しました。
* AI: AI プロンプト: MongoDBスキーマ変更前に作成されたアカウントで、エラー表示ではなく空白画面になる問題を修正しました。
* AI: Ask AI: デスクトップでエラーの解決を尋ねるときに外部ブラウザではなくデスクトップ上のチャットクライアントを開くようになりました。


* テーブルビュー: 他のステップに移動しても列のサマリー情報のパネルが閉じない問題を修正しました。


* データラングリング: カイ二乗検定ダイアログで文字列型列を選択できない問題を修正しました。
* データラングリング: Google Sheetsへのエクスポート時に先頭のゼロが失われる問題を修正しました。
* データラングリング: 論理値引数'FALSE'がシンタックスサジェストで適切に提案されない問題を修正しました。


* チャート: 箱ひげ図: X軸の「使用されていないレベルを表示」設定が正しく機能しない問題を修正しました。


* アナリティクス: t検定: ステップで作成された結果と異なる結果が出る問題を修正しました。
...
* 一般: Mac: Cmd + Backtick（`）でウィンドウを切り替えたときにウィンドウが移動しないようにしました。
* 一般: プロジェクトを開いたときにpurrrパッケージがデフォルトでロードされるようになりました。
