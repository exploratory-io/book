...
* データラングリング: 列ヘッダメニュー内の項目をクリックした後、メニューが閉じなくなってしまう。
* データラングリング: 他の列ヘッダメニューをクリックしても、元々開いていた列ヘッダメニューが閉じない。
* データラングリング: データフレームを選択した際に、「r.indexOf is not a function」というエラーが表示されることがある。


* チャート: 余白の自動調整のオプションが有効になっている場合、ユーザーが指定した余白の値が機能しない。
* チャート: パイ / ドーナツチャートのデフォルトの余白の設定を更新し、チャート上の中央に表示されるようになります。
* チャート: ナンバー: 比較の対象が設定されていない場合、基準値は表示されるべきではありません。
* チャート: チャート内の全画面表示をクリックした時に、詳細データの表示ダイアログが表示されない。
* チャート: チャートイメージのエクスポートでは、ピクセル比率のデフォルト値は常に300%である必要があります。


* ノート: ツールバーの引用符アイコンをクリックして、通常のテキストと引用を切り替えられるようになりました。
* ノート: 箇条書きの行を選択した状態で、ツールバーの箇条書きアイコンをクリックすると、箇条書きが無効化されるようになりました。
* ノート: テキストを選択した状態でツールバーのコードブロック・アイコンをクリックすると、選択していたテキストが削除されてしまう。
* ノート: プレビュー前に、EDFをエクスポートすると失敗する。
* ノート: ヘッダーがないときには、プレビュー画面に目次領域が表示されなくなりました。
...
* ノート: プレビューボタンに再生アイコンが追加されました。
* ノート: チャートのツールバーが、チャートの左上に表示されるようになりました。
* ノート: パイチャートで値を表示したときに、上部の値が見切れてしまう。
* ノート: ノート内のテキストが多いときに矢印キーでカーソルを動かすと移動に時間がかかる。
* ノート: 複数の画像を挿入しているときに、矢印キーで上下に移動すると、急激なスクロールが発生する。
* ノート: コピー＆ペーストしたテキストをヘッダーに設定すると、1行目だけでなく、ペーストした全てのテキストがヘッダーになってしまう。
* ノート: 「LaTeX書式を有効にする」オプションを使って、'$'記号で囲まれたテキストが、インラインの数式として扱われることを防げるようになりました。
* ノート: 「LaTeX書式を有効にする」オプションのデフォルト値が、「無効」になりました。
* ノート: 外部ブラウザからテキストをコピー＆ペーストしたときに、「#」「.」「*」のようなマークダウンの構文で利用されている特殊文字が含まれていると、テキストが適切にコピーされない。


* ダッシュボード: ダッシュボードに1ページしかない場合、ページ名の変更が機能しない。
* ダッシュボード: ピボット: 値にの大きさをバーで表現する列のセルをクリックした時に、レイアウトが壊れる。
* ダッシュボード: サイドバーからダッシュボードにマウスオーバーた時に表示されるポップアップに、上下の余白が余分に表示されている。
* ダッシュボード: インタラクティブ・フィルターを適用した時に、「Loading...」のテキストの表示の代わりに、実行中のアイコンを表示するべきです。


* プロジェクト: Windowsでプロジェクトを開く時に、エラーコード 127でプロジェクトを開けないことがある。
* プロジェクト: プロジェクトをエクスポートする際に、EPFファイルの名称にシングルクォーテーションが含まれている場合、「line 1: syntax error: unexpected end of file」というエラーでエクスポートに失敗する。
* プロジェクト: プロジェクトのヘッダメニューに「名前と概要の編集」を追加しました。
