...
* チャート: 基準線: 累積合計 / 累積比率および移動平均の計算が散布図・バブルチャートで正しくありませんでした。
* チャート: 基準線: Y に移動平均、X と Y に特定値を追加するとエラーが発生しました。
* チャート: 基準線: 2本目のラインのラベルが「凡例に表示」をオンにしても表示されませんでした。
* チャート: 基準線: ロジカル型のデータタイプの列の時に Y 軸スケールが正しくありませんでした。
* チャート: リファレンスライン: 累積合計値の割合の基準線設定時、Y2 軸タイトルとラベルが重なり、ラベルが切れていました。
* チャート: 散布図: X 軸に日付列を割り当てた場合、基準線が表示されませんでした。
...
* アナリティクス: 教師あり学習モデル: 予測変数として日付列を使用すると、日の値が誤ってカテゴリとして扱われました。
...
* パブリッシュ: 2GBを超えるサイズのデータのパブリッシュ時に “paste(csv_content...) result would exceed 2^31-1 bytes” というエラーが発生しました。


* プロジェクト: 重複: 無効なデータフレームメタデータを含むプロジェクトを複製するとエラーが発生しました。
* プロジェクト: プロジェクトウィンドウが1回のクリックで閉じませんでした。
