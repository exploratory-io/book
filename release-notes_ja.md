...
* データラングリング: 正規表現ヘルプのURLリンクを変更しました。
* データラングリング: サマリーポップアップに「チャート」ボタンを追加しました。
* データラングリング: サマリーのNAラベルに「欠損値」を追加しました。

* チャート: エリア/バー: 「スタック」と「オーバーレイ」の日本語ラベルを更新しました。
* チャート: 参照線: 左ツールメニューで無効化された参照線をグレーアウトする機能を追加しました。

* アナリティクス: ランダムフォレスト: テンプレートにROC曲線セクションを追加しました。

* ノート: AIノートエディター: AI用のCSVデータ生成中のスピナー表示を統合しました。

* 一般: すべてのファイルエクスポート操作にフォールバックエラー処理を追加しました。
