...
* データソース: Google BigQuery: サービスアカウントのサポートが追加されました。
* アナリティクス: AIサマリ: サマリの内容を簡単に共有できるよう、テキストコピーボタンが追加されました。

### 修正された問題

* データソース: Google Analytics: 旧バージョンのv3 GAデータソースを再インポートする際に、ユーザーフレンドリーなメッセージが表示されるようになりました。

* データラングリング: EDF以外のファイルを選択する際に、ユーザーフレンドリーなメッセージが表示されるようになりました。

* チャート: 円グラフ/ドーナツグラフ: 個々の色の透明度を調整できない問題を修正しました。

* アナリティクス: 統計モデル: 繰り返し処理を使用している際に、完全共線性のあるグループが多重共線性チャートに表示されない問題を修正しました。
* アナリティクス: ウィンドウ間を移動する際に、アナリティクスレポートが上部にスクロールバックされてしまう問題を修正しました。
* アナリティクス: 単語のカウント: パブリッシュされた単語のカウントアナリティクスで、共起チャートが表示されない問題を修正しました。
* アナリティクス: 線形回帰: パラメーターからデータをダウンロードする際に、残差チャートが間違ってエクスポートされる問題を修正しました。

* ダッシュボード: 多数のチャートを含むダッシュボードでの作業時のパフォーマンスが改善されました。
* ダッシュボード: 特定の条件下で発生していた「ダッシュボード生成中に変数の更新に失敗」エラーを修正しました。

* ノート: Rスクリプト: ggplot2チャートのズーム比率がウェブとデスクトップのプレビューで異なる問題を修正しました。

* パラメーター: 「値をキャッシュ」ラベルを「動的に値を取得」に変更し、明確化のためデフォルトをオフに設定しました。

* 履歴: 「認識されない入力による履歴の復元に失敗」というエラーを修正しました。
